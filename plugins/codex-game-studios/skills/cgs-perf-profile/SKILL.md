...
description: "Codex Game Studios skill adapted from original /perf-profile. Use when the user asks for /perf-profile, $cgs-perf-profile, or this workflow. Structured performance profiling workflow. Identifies bottlenecks, measures against budgets, and generates optimization recommendations with priority rankings."
---

# CGS: perf-profile

> Codex adaptation: this skill is migrated from the upstream `/perf-profile` workflow. Invoke it as `$cgs-perf-profile`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- **B) Reduce feature scope** (run `$cgs-scope-check [feature]` to analyze trade-offs)
- **C) Accept the performance hit and defer to Polish phase** (log as known issue)
- **D) Escalate to technical-director for an architectural decision** (run `$cgs-architecture-decision`)
...
- If bottlenecks require architectural change: run `$cgs-architecture-decision`.
- If scope reduction is needed: run `$cgs-scope-check [feature]`.
- To schedule optimizations: run `$cgs-sprint-plan update`.
