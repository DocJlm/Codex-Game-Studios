...
description: "Codex Game Studios skill adapted from original /quick-design. Use when the user asks for /quick-design, $cgs-quick-design, or this workflow. Lightweight design spec for small changes -- tuning adjustments, minor mechanics, balance tweaks. Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files."
---
...
Full GDD authoring via `$cgs-design-system` is the heavyweight path. Use this skill
...
**When to run:** Anytime a change is too small for `$cgs-design-system` but too
...
and redirect to `$cgs-design-system` instead.
...
- Options:
  ...
  - `[F] This is too large -- redirect me to $cgs-design-system`

If [F]: stop. Verdict: **REDIRECTED** -- use `$cgs-design-system` for this change.
...
- Options:
  ...
  - `[C] This grew too large -- redirect to $cgs-design-system instead`

If [B]: collect the requested changes, revise the draft, and re-present this widget.
If [C]: stop. Verdict: **REDIRECTED** -- use `$cgs-design-system` for this change.
...
Next step: This spec is ready for `$cgs-story-readiness` validation before
...
Quick Design Specs **bypass** `$cgs-design-review` and `$cgs-review-all-gdds` by
...
using `$cgs-design-system` to author a full GDD for this."

---

## Recommended Next Steps

- Run `$cgs-story-readiness [story-path]` to validate the story before implementation begins -- reference this spec in the story's GDD Reference field
- Run `$cgs-dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `$cgs-design-system [system-name]` to author a full GDD instead
