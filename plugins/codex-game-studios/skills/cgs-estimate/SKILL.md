---
name: cgs-estimate
description: "Codex Game Studios skill adapted from original /estimate. Use when the user asks for /estimate, $cgs-estimate, or this workflow. Estimates task effort by analyzing complexity, dependencies, historical velocity, and risk factors. Produces a structured estimate with confidence levels."
---

# CGS: estimate

> Codex adaptation: this skill is migrated from the upstream `/estimate` workflow. Invoke it as `$cgs-estimate`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- If confidence is Low: recommend a time-boxed spike (`$cgs-prototype`) before committing.
- If the task is > 10 days: recommend breaking it into smaller stories via `$cgs-create-stories`.
- To schedule the task: run `$cgs-sprint-plan update` to add it to the next sprint.
