...
> Codex adaptation: this skill is migrated from the upstream `/team-level` workflow. Invoke it as `$cgs-team-level`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Gate**: Use `ask the user directly or use available Codex UI question tools` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
**Gate**: Use `ask the user directly or use available Codex UI question tools` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `ask the user directly or use available Codex UI question tools` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `ask the user directly or use available Codex UI question tools` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
