...
description: "Codex Game Studios skill adapted from original /scope-check. Use when the user asks for /scope-check, $cgs-scope-check, or this workflow. Analyze a feature or sprint for scope creep by comparing current scope against the original plan. Flags additions, quantifies bloat, and recommends cuts. Use when user says 'any scope creep', 'scope review', 'are we staying in scope'."
---

# CGS: scope-check

> Codex adaptation: this skill is migrated from the upstream `/scope-check` workflow. Invoke it as `$cgs-scope-check`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- **CONCERNS** -> offer to identify the 2-3 additions with best cut ratio. Reference `$cgs-sprint-plan update` to formally re-scope.
- **FAIL** -> recommend escalating to producer. Reference `$cgs-sprint-plan update` for re-planning or `$cgs-estimate` to re-baseline timeline.

Always end with:
> "Run `$cgs-scope-check [name]` again after cuts are made to verify the verdict improves."
