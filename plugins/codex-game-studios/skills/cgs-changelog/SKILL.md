---
name: cgs-changelog
description: "Codex Game Studios skill adapted from original /changelog. Use when the user asks for /changelog, $cgs-changelog, or this workflow. Auto-generates a changelog from git commits, sprint data, and design documents. Produces both internal and player-facing versions."
---

# CGS: changelog

> Codex adaptation: this skill is migrated from the upstream `/changelog` workflow. Invoke it as `$cgs-changelog`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- Use `$cgs-patch-notes [version]` to generate a styled, saved version for public release.
- Use `$cgs-release-checklist` before publishing the changelog externally.
