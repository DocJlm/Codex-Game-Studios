...
description: "Codex Game Studios skill adapted from original /regression-suite. Use when the user asks for /regression-suite, $cgs-regression-suite, or this workflow. Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate."
---

# CGS: regression-suite

> Codex adaptation: this skill is migrated from the upstream `/regression-suite` workflow. Invoke it as `$cgs-regression-suite`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- Before a release gate (`$cgs-gate-check polish` requires regression suite exists)
...
**Modes:**
- `$cgs-regression-suite update` -- scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `$cgs-regression-suite audit` -- full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `$cgs-regression-suite report` -- read-only status report (no writes); suitable
  for sprint reviews
- No argument -- if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `ask the user directly or use available Codex UI question tools`:
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `$cgs-test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `$cgs-regression-suite audit` at the next sprint boundary."
