---
name: cgs-test-setup
description: "Codex Game Studios skill adapted from original /test-setup. Use when the user asks for /test-setup, $cgs-test-setup, or this workflow. Scaffold the test framework and CI/CD pipeline for the project's engine. Creates the tests/ directory structure, engine-specific test runner configuration, and GitHub Actions workflow. Run once during Technical Setup phase before the first sprint begins."
---

# CGS: test-setup

> Codex adaptation: this skill is migrated from the upstream `/test-setup` workflow. Invoke it as `$cgs-test-setup`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
1. **Read engine config**:
   - Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and extract the `Engine:` value.
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `$cgs-setup-engine` first, then re-run `$cgs-test-setup`."
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `$cgs-test-setup force`
...
tests/
  ...
  smoke/          # Critical path test list for $cgs-smoke-check gate
...
```gdscript
# GdUnit4 test runner -- invoked by CI and $cgs-smoke-check
...
**Run via**: `$cgs-smoke-check` (which reads this file)
...
3. Run `$cgs-qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `$cgs-smoke-check` before every QA hand-off

Gate note: $cgs-gate-check Technical Setup -> Pre-Production now requires:
- tests/ directory with unit/ and integration/ subdirectories
- .github/workflows/tests.yml
- At least one example test file
Run $cgs-test-setup and write one example test before advancing.
...
- **Engine detection is non-negotiable** -- if the engine is not configured,
  stop and redirect to `$cgs-setup-engine`. Do not guess.
