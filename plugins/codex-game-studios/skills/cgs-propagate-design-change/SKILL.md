...
description: "Codex Game Studios skill adapted from original /propagate-design-change. Use when the user asks for /propagate-design-change, $cgs-propagate-design-change, or this workflow. When a GDD is revised, scans all ADRs and the traceability index to identify which architectural decisions are now potentially stale. Produces a change impact report and guides the user through resolution."
---
...
**Usage:** `$cgs-propagate-design-change design/gdd/combat-system.md`
...
> "Usage: `$cgs-propagate-design-change design/gdd/[system].md`
...
|  **Needs Review** | The GDD change may affect this ADR -- human judgment needed |
...
- **ADRs marked Superseded**: "Run `$cgs-architecture-decision [title]` to write the
  replacement ADR. Then re-run `$cgs-propagate-design-change` to verify coverage."
- **ADRs to update in place**: List the specific fields to update in each ADR
- **If many ADRs affected**: "Run `$cgs-architecture-review` after all ADRs are updated
