...
description: "Codex Game Studios skill adapted from original /test-evidence-review. Use when the user asks for /test-evidence-review, $cgs-test-evidence-review, or this workflow. Quality review of test files and manual evidence documents. Goes beyond existence checks -- evaluates assertion coverage, edge case handling, naming conventions, and evidence completeness. Produces ADEQUATE/INCOMPLETE/MISSING verdict per story. Run before QA sign-off or on demand."
---

# CGS: test-evidence-review

> Codex adaptation: this skill is migrated from the upstream `/test-evidence-review` workflow. Invoke it as `$cgs-test-evidence-review`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
`$cgs-smoke-check` verifies that test files **exist** and **pass**. This skill
...
**When to run:**
- Before QA hand-off sign-off (`$cgs-team-qa` Phase 5)
...
**Modes:**
- `$cgs-test-evidence-review [story-path]` -- review a single story's evidence
- `$cgs-test-evidence-review sprint` -- review all stories in the current sprint
- `$cgs-test-evidence-review [system-name]` -- review all stories in an epic/system
...
- For BLOCKING items: "These must be resolved before `$cgs-story-done` can mark the
  story Complete. Would you like to address any of them now?"
- For thin assertions: "Consider running `$cgs-test-helpers [system]` to see
