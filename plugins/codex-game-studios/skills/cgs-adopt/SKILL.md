---
name: cgs-adopt
description: "Codex Game Studios skill adapted from original /adopt. Use when the user asks for /adopt, $cgs-adopt, or this workflow. Brownfield onboarding -- audits existing project artifacts for template format compliance (not just existence), classifies gaps by impact, and produces a numbered migration plan. Run this when joining an in-progress project or upgrading from an older template version. Distinct from $cgs-project-stage-detect (which checks what exists) -- this checks whether what exists will actually work with the template's skills."
---
...
**This is not `$cgs-project-stage-detect`.**
`$cgs-project-stage-detect` answers: *what exists?*
`$cgs-adopt` answers: *will what exists actually work with the template's skills?*
...
- `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` -- engine configured?
...
Use the same heuristic as `$cgs-project-stage-detect`:
...
|---|---|
| `## Status` | **BLOCKING** -- `$cgs-story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH -- dependency ordering in `$cgs-architecture-review` breaks |
...
1. **Parenthetical status values** -- Grep for any Status cell containing
   ...
   These break exact-string matching in `$cgs-gate-check`, `$cgs-create-stories`,
   and `$cgs-architecture-review`. **BLOCKING.**
...
| Sprint status | `production/sprint-status.yaml` | MEDIUM -- `$cgs-sprint-status` falls back to markdown |
...
Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
`$cgs-architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`$cgs-design-system retrofit design/gdd/[filename].md`
...
2. Run `$cgs-architecture-review` -> bootstraps `tr-registry.yaml`
3. Run `$cgs-create-control-manifest` -> creates manifest with version stamp
4. Run `$cgs-sprint-plan update` -> creates `sprint-status.yaml`
5. Run `$cgs-gate-check [phase]` -> writes `stage.txt` authoritatively
...
Gap counts:
  ...
  HIGH:     [N] -- unsafe to run $cgs-create-stories or $cgs-story-readiness
  MEDIUM:   [N] -- quality degradation
  LOW:      [N] -- optional improvements

Estimated remediation: [X blocking items x ~Y min each = roughly Z hours]
```
...
Re-run `$cgs-adopt` anytime to check remaining gaps.
...
Run `$cgs-architecture-review` -- even if ADRs already exist, this run bootstraps
...
Run `$cgs-gate-check [current-phase]`
**Time**: 5 min
...
Run `$cgs-adopt` again after completing Step 3 to verify all blocking and high gaps
...
- **Options**:
  ...
  - `Lean (recommended)` -- Directors only at phase gate transitions ($cgs-gate-check). Skips per-skill reviews. Balanced for solo devs and small teams.
...
> **Adoption plan saved to `docs/adoption-plan-[date].md`.** Re-run `$cgs-adopt` at any time to re-check remaining gaps as you complete them.
