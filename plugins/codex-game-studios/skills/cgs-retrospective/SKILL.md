...
description: "Codex Game Studios skill adapted from original /retrospective. Use when the user asks for /retrospective, $cgs-retrospective, or this workflow. Generates a sprint or milestone retrospective by analyzing completed work, velocity, blockers, and patterns. Produces actionable insights for the next iteration."
---
...
If the user selects [A]: Proceed to invoke `$cgs-sprint-plan new`, passing the retrospective file path and a summary of the action items and velocity change so the sprint planner can reference them.

- If this was a milestone retrospective, run `$cgs-gate-check` to formally assess readiness for the next phase.
