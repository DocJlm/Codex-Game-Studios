...
description: "Codex Game Studios skill adapted from original /architecture-review. Use when the user asks for /architecture-review, $cgs-architecture-review, or this workflow. Validates completeness and consistency of the project architecture against all GDDs. Builds a traceability matrix mapping every GDD technical requirement to ADRs, identifies coverage gaps, detects cross-ADR conflicts, verifies engine compatibility consistency across all decisions, and produces a PASS/CONCERNS/FAIL verdict. The architecture equivalent of $cgs-design-review."
---

# CGS: architecture-review

> Codex adaptation: this skill is migrated from the upstream `/architecture-review` workflow. Invoke it as `$cgs-architecture-review`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
### Project Standards
- `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`
...
|  **Partial** | An ADR partially covers this, or coverage is ambiguous |
...
3. **Flag unresolved dependencies**: If ADR-A's "Depends On" field references an ADR
   ...
   ```
     ADR-0005 depends on ADR-0002 -- but ADR-0002 is still Proposed.
...
If [B]: display the complete proposed systems-index section, then re-ask with `ask the user directly or use available Codex UI question tools`.
...
For each gap:
   TR-[id]: [GDD] -> [system] -> [requirement]
     Suggested ADR: "$cgs-architecture-decision [suggested title]"
...
Use `ask the user directly or use available Codex UI question tools` for the write approval:
...
For `rtm` mode, use `ask the user directly or use available Codex UI question tools`:
...
> Last Updated: [date]
> Mode: $cgs-architecture-review rtm
...
```markdown
### [YYYY-MM-DD] -- $cgs-architecture-review --  CONFLICT
...
`production/session-state/active.md`:
    ...
    ## Session Extract -- $cgs-architecture-review [date]
    - Verdict: [PASS / CONCERNS / FAIL]
...
2. **Pre-gate checklist**: Check whether these exist via Glob and mark each  or :
   - `tests/unit/` and `tests/integration/` directories -- if : run `$cgs-test-setup`
   - `.github/workflows/tests.yml` -- if : run `$cgs-test-setup`
   - `design/accessibility-requirements.md` -- if : run `$cgs-ux-design`
   - `design/ux/interaction-patterns.md` -- if : run `$cgs-ux-design`
   Present  items as required steps before gate-check. Do not offer `$cgs-gate-check`
   as an option if any item is  -- offer the missing skill to run instead.
3. **Rerun trigger**: "Re-run `$cgs-architecture-review` after each new ADR is written
   to verify coverage improves"

Then close with `ask the user directly or use available Codex UI question tools` tailored to the pre-gate checklist state:
- If ADR gaps remain or any pre-gate item is :
  - "Architecture review complete. What would you like to do next?"
    - [A] Write a missing ADR -- open a fresh session and run `$cgs-architecture-decision [system]`
    - [B] Run `$cgs-test-setup` -- required before gate-check (only show if test infrastructure is )
    - [C] Run `$cgs-ux-design` -- required before gate-check (only show if UX/accessibility files are )
...
- If all pre-gate checklist items are  and no blocking ADR gaps remain:
  - "Architecture review complete. All pre-gate items confirmed. What would you like to do next?"
    - [A] Run `$cgs-gate-check pre-production`
    - [B] Write a missing ADR -- open a fresh session and run `$cgs-architecture-decision [system]`
