---
name: cgs-gate-check
description: "Codex Game Studios skill adapted from original /gate-check. Use when the user asks for /gate-check, $cgs-gate-check, or this workflow. Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts. Use when user says 'are we ready to move to X', 'can we advance to production', 'check if we can start the next phase', 'pass the gate'."
---

# CGS: gate-check

> Codex adaptation: this skill is migrated from the upstream `/gate-check` workflow. Invoke it as `$cgs-gate-check`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.

> Migration phase: Core workflow MVP. Legacy role names are available as role cards under `plugins/codex-game-studios/references/role-cards/`.
...
**Distinct from `$cgs-project-stage-detect`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `$cgs-gate-check production` -- validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `$cgs-project-stage-detect`, then **confirm with the user before running**:
...
- [ ] Concept prototype exists in `prototypes/` with a REPORT.md showing PROCEED verdict
      (`$cgs-prototype [core-mechanic]`) -- skipping this means GDDs may be written for an
      idea that hasn't been played. Acceptable if the concept is proven by other means.

**Quality Checks:**
- [ ] Game concept has been reviewed (`$cgs-design-review` verdict not MAJOR REVISION NEEDED)
...
- [ ] All MVP-tier GDDs exist in `design/gdd/` and individually pass `$cgs-design-review`
- [ ] A cross-GDD review report exists in `design/gdd/` (from `$cgs-review-all-gdds`)

**Quality Checks:**
- [ ] All MVP GDDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `$cgs-review-all-gdds` verdict is not FAIL (cross-GDD consistency and design theory checks pass)
- [ ] All cross-GDD consistency issues flagged by `$cgs-review-all-gdds` are resolved or explicitly accepted
...
- [ ] Technical preferences configured (`plugins/codex-game-studios/references/studio-docs/technical-preferences.md` populated)
...
- [ ] `$cgs-architecture-review` has been run (a review report file exists in `docs/architecture/`)
...
- [ ] `$cgs-architecture-review` engine audit shows no deprecated API usage
...
**Required Artifacts:**
- [ ] Vertical slice exists in `prototypes/` with a REPORT.md (run `$cgs-vertical-slice`) -- **recommended, not blocking**; if absent, surface as CONCERNS
...
- [ ] Entity inventory exists at `design/assets/entity-inventory.md` (recommended -- run `$cgs-asset-spec` with no arguments to generate collaboratively from GDDs + art bible)
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated by `$cgs-create-control-manifest` from Accepted ADRs)
- [ ] Epics defined in `production/epics/` with at least Foundation and Core
      layer epics present (use `$cgs-create-epics layer: foundation` and
      `$cgs-create-epics layer: core` to create them, then `$cgs-create-stories [epic-slug]`
      for each epic)
...
- [ ] All key screen UX specs have passed `$cgs-ux-review` (verdict APPROVED or NEEDS REVISION accepted)
...
- [ ] Manual validation confirms GDDs + architecture + epics are coherent
      (run `$cgs-review-all-gdds` and `$cgs-architecture-review` if not done recently)
...
- [ ] QA plan exists in `production/qa/` (generated by `$cgs-qa-plan`) covering this sprint or final production sprint
- [ ] At least one QA plan exists in `production/qa/` covering this production phase -- run `$cgs-qa-plan` if missing (CONCERNS -- advisory, not blocking)
- [ ] QA sign-off report exists in `production/qa/` (generated by `$cgs-team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
- [ ] QA test plan exists (`$cgs-qa-plan` output in `production/qa/`)
- [ ] QA sign-off report exists (`$cgs-team-qa` output -- APPROVED or APPROVED WITH CONDITIONS)
...
- [ ] Balance data has been reviewed (`$cgs-balance-check` run)
- [ ] Release checklist completed (`$cgs-release-checklist` or `$cgs-launch-checklist` run)
...
Use `Glob('design/gdd/gdd-cross-review-*.md')` to find the `$cgs-review-all-gdds` report.
If no file matches, mark the "cross-GDD review report exists" artifact as **FAIL** and
surface it prominently: "No `$cgs-review-all-gdds` report found in `design/gdd/`. Run
`$cgs-review-all-gdds` before advancing to Technical Setup."
...
- For performance checks: `Read` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent `$cgs-perf-profile` output
...
- "Performance profiling data isn't available. Would you like to run `$cgs-perf-profile`?"
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Task using the parallel gate protocol from `plugins/codex-game-studios/references/studio-docs/director-gates.md`. Issue all four Task calls simultaneously -- do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** -- gate **CD-PHASE-GATE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
2. **`technical-director`** -- gate **TD-PHASE-GATE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
3. **`producer`** -- gate **PR-PHASE-GATE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
4. **`art-director`** -- gate **AD-PHASE-GATE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
...
### Blockers
1. **No Architecture Decision Records** -- Run `$cgs-architecture-decision` to create one
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `ask the user directly or use available Codex UI question tools`.
...
[A] Run $cgs-create-architecture -- produce your master architecture blueprint and ADR work plan (recommended next step)
[B] Design more GDDs first -- return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `$cgs-create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `$cgs-architecture-decision` without this step means writing ADRs without a blueprint -- skip it at your own risk.

For **technical-setup PASS**:
```
Gate passed. What would you like to do next?
[A] Run $cgs-create-control-manifest -- generate the layer rules manifest from your Accepted ADRs (do this first)
[B] Run $cgs-vertical-slice -- build the Vertical Slice (do this before writing epics -- validate fun first)
[C] Write more ADRs first -- run $cgs-architecture-decision [next-system]
...
>
> 1. `$cgs-create-control-manifest` -- extract technical rules from Accepted ADRs (required before epics)
> 2. `$cgs-vertical-slice` -- build the Vertical Slice **FIRST**, before writing epics or stories
> 3. Playtest -> `$cgs-playtest-report` -- at least 1 session required to pass the Pre-Production gate; 3+ recommended before committing the full team
> 4. `$cgs-ux-design [screen]` -- UX specs for main menu, core HUD, pause menu (if not done)
> 5. `$cgs-create-epics layer:foundation` then `$cgs-create-epics layer:core` -- plan after fun is validated
> 6. `$cgs-create-stories [epic-slug]` for each epic
> 7. `$cgs-sprint-plan new`
>
...
- **No art bible?** -> `$cgs-art-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** -> `$cgs-asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved GDDs
- **No game concept?** -> `$cgs-brainstorm` to create one
- **No systems index?** -> `$cgs-map-systems` to decompose the concept into systems
- **Missing design docs?** -> `$cgs-reverse-document` or delegate to `game-designer`
- **Small design change needed?** -> `$cgs-quick-design` for changes under ~4 hours (bypasses full GDD pipeline)
- **No UX specs?** -> `$cgs-ux-design [screen name]` to author specs, or `$cgs-team-ui [feature]` for full pipeline
- **UX specs not reviewed?** -> `$cgs-ux-review [file]` or `$cgs-ux-review all` to validate
- **No accessibility requirements doc?** -> run `$cgs-ux-design` which creates both `design/accessibility-requirements.md` and `design/ux/interaction-patterns.md` in one step
- **No interaction pattern library?** -> `$cgs-ux-design patterns` to initialize it
- **GDDs not cross-reviewed?** -> `$cgs-review-all-gdds` (run after all MVP GDDs are individually approved)
- **Cross-GDD consistency issues?** -> fix flagged GDDs, then re-run `$cgs-review-all-gdds`
- **No test framework?** -> `$cgs-test-setup` to scaffold the framework for your engine
- **No QA plan for current sprint?** -> `$cgs-qa-plan sprint` to generate one before implementation begins
- **Missing ADRs?** -> `$cgs-architecture-decision` for individual decisions
- **No master architecture doc?** -> `$cgs-create-architecture` for the full blueprint
- **ADRs missing engine compatibility sections?** -> Re-run `$cgs-architecture-decision`
  or manually add Engine Compatibility sections to existing ADRs
- **Missing control manifest?** -> `$cgs-create-control-manifest` (requires Accepted ADRs)
- **Missing epics?** -> `$cgs-create-epics layer: foundation` then `$cgs-create-epics layer: core` (requires control manifest)
- **Missing stories for an epic?** -> `$cgs-create-stories [epic-slug]` (run after each epic is created)
- **Stories not implementation-ready?** -> `$cgs-story-readiness` to validate stories before developers pick them up
- **Tests failing?** -> delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** -> `$cgs-playtest-report`
- **No playtest sessions beyond the minimum?** -> Additional sessions give more reliable signal. 3+ total is recommended before committing the full team. Use `$cgs-playtest-report` to structure findings.
- **No Difficulty Curve doc?** -> Create `design/difficulty-curve.md` from the template at `plugins/codex-game-studios/assets/templates/difficulty-curve.md` -- or use `$cgs-quick-design "difficulty curve"` for a guided session.
- **No player journey map?** -> Create `design/player-journey.md` from the template at `plugins/codex-game-studios/assets/templates/player-journey.md` -- or author it collaboratively using `$cgs-ux-design` Phase 2b.
- **Need a quick sprint check?** -> `$cgs-sprint-status` for current sprint progress snapshot
- **Performance unknown?** -> `$cgs-perf-profile`
- **Not localized?** -> `$cgs-localize`
- **Ready for release?** -> `$cgs-launch-checklist`
...
6. **Never auto-fix**: If required artifacts are missing, report the FAIL verdict and
   name the skill to run (e.g. "run `$cgs-test-setup`"). Do NOT create missing files or
