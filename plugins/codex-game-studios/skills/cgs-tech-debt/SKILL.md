---
name: cgs-tech-debt
description: "Codex Game Studios skill adapted from original /tech-debt. Use when the user asks for /tech-debt, $cgs-tech-debt, or this workflow. Track, categorize, and prioritize technical debt across the codebase. Scans for debt indicators, maintains a debt register, and recommends repayment scheduling."
---
...
Score each item by: `(impact_if_unfixed x frequency_of_encounter) / fix_effort`
...
- Run `$cgs-sprint-plan` to schedule high-priority debt items into the next sprint.
- Run `$cgs-tech-debt report` at the start of each sprint to track debt trends over time.
