...
description: "Codex Game Studios skill adapted from original /patch-notes. Use when the user asks for /patch-notes, $cgs-patch-notes, or this workflow. Generate player-facing patch notes from git history, sprint data, and internal changelogs. Translates developer language into clear, engaging player communication."
---

# CGS: patch-notes

> Codex adaptation: this skill is migrated from the upstream `/patch-notes` workflow. Invoke it as `$cgs-patch-notes`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- Read the internal changelog at `production/releases/[version]$cgs-changelog.md` if it exists
...
**If no changelog data is available** (neither `production/releases/[version]$cgs-changelog.md`
nor a `docs/CHANGELOG.md` entry for this version exists, and git log is empty or unavailable):

> "No changelog data found for [version]. Run `$cgs-changelog [version]` first to generate the
> internal changelog, then re-run `$cgs-patch-notes [version]`."
...
1. Check `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` for any "tone", "voice", or "style"
   fields or sections.
...
1. Glob for `docs/patch-notes-template.md` and `plugins/codex-game-studios/assets/templates/patch-notes-template.md`.
...
if needed. Also write to `production/releases/[version]$cgs-patch-notes.md` as the
...
- Run `$cgs-release-checklist` to verify all other release gates are met before publishing.
