---
name: cgs-bug-report
description: "Codex Game Studios skill adapted from original /bug-report. Use when the user asks for /bug-report, $cgs-bug-report, or this workflow. Creates a structured bug report from a description, or analyzes code to identify potential bugs. Ensures every bug report has full reproduction steps, severity assessment, and context."
---

# CGS: bug-report

> Codex adaptation: this skill is migrated from the upstream `/bug-report` workflow. Invoke it as `$cgs-bug-report`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
If STILL PRESENT: reopen the bug, set Status back to Open, and suggest re-running `$cgs-hotfix [BUG-ID]`.

---

## Phase 2D: Close Mode

Read `production/qa/bugs/[BUG-ID].md`. Confirm Status is `Verified Fixed` before closing. If status is anything else, stop: "Bug [ID] must be Verified Fixed before it can be closed. Run `$cgs-bug-report verify [BUG-ID]` first."
...
After closing, check `production/qa/bug-triage-*.md` -- if the bug appears in an open triage report, note: "Bug [ID] is referenced in the triage report. Run `$cgs-bug-triage` to refresh the open bug count."
...
- Run `$cgs-bug-triage` to prioritize alongside existing open bugs
- If S1 or S2: run `$cgs-hotfix [BUG-ID]` for emergency fix workflow

**After fixing the bug (developer confirms fix is in):**
- Run `$cgs-bug-report verify [BUG-ID]` -- confirm the fix actually works before closing
- Never mark a bug closed without verification -- a fix that doesn't verify is still Open

**After verify returns VERIFIED FIXED:**
- Run `$cgs-bug-report close [BUG-ID]` -- write the closure record and update status
- Run `$cgs-bug-triage` to refresh the open bug count and remove it from the active list
