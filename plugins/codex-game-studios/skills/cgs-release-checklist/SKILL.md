...
description: "Codex Game Studios skill adapted from original /release-checklist. Use when the user asks for /release-checklist, $cgs-release-checklist, or this workflow. Generates a comprehensive pre-release validation checklist covering build verification, certification requirements, store metadata, and launch readiness."
---

# CGS: release-checklist

> Codex adaptation: this skill is migrated from the upstream `/release-checklist` workflow. Invoke it as `$cgs-release-checklist`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `$cgs-release-checklist`. Do not auto-invoke based on context matching.
...
- Run `$cgs-gate-check` for a formal phase gate verdict before proceeding to release.
- Coordinate final sign-offs via `$cgs-team-release`.
