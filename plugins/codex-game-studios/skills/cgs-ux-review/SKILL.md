---
name: cgs-ux-review
description: "Codex Game Studios skill adapted from original /ux-review. Use when the user asks for /ux-review, $cgs-ux-review, or this workflow. Validates a UX spec, HUD design, or interaction pattern library for completeness, accessibility compliance, GDD alignment, and implementation readiness. Produces APPROVED / NEEDS REVISION / MAJOR REVISION NEEDED verdict with specific gaps."
---

# CGS: ux-review

> Codex adaptation: this skill is migrated from the upstream `/ux-review` workflow. Invoke it as `$cgs-ux-review`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
the `$cgs-team-ui` pipeline.

**Run this skill:**
- After completing a UX spec with `$cgs-ux-design`
...
- **Specific file path** (e.g., `$cgs-ux-review design/ux/inventory.md`): validate
  that one document
...
1. **Input & Platform config**: Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and
...
[For APPROVED]: This spec is ready for handoff to `$cgs-team-ui` Phase 2
(Visual Design).
...
Recommend returning to `$cgs-ux-design` to rework [sections].
```
...
- For **APPROVED**: suggest running `$cgs-team-ui` to begin implementation coordination
- For **NEEDS REVISION**: offer to help fix specific gaps ("Would you like me to
  ...
  instruction
- For **MAJOR REVISION NEEDED**: suggest returning to `$cgs-ux-design` with the
