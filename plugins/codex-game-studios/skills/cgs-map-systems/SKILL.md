...
description: "Codex Game Studios skill adapted from original /map-systems. Use when the user asks for /map-systems, $cgs-map-systems, or this workflow. Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index."
---

# CGS: map-systems

> Codex adaptation: this skill is migrated from the upstream `/map-systems` workflow. Invoke it as `$cgs-map-systems`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- **No argument**: `$cgs-map-systems` -- Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `$cgs-map-systems next` -- Pick the highest-priority undesigned system
  from the index and hand off to `$cgs-design-system` (Phase 6).
...
- Read `design/gdd/game-concept.md` -- **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `$cgs-brainstorm` first
...
- Use `ask the user directly or use available Codex UI question tools` to ask:
...
Then use `ask the user directly or use available Codex UI question tools` to capture feedback:
...
Use `ask the user directly or use available Codex UI question tools` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`plugins/codex-game-studios/references/studio-docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `ask the user directly or use available Codex UI question tools` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`plugins/codex-game-studios/references/studio-docs/director-gates.md`) before writing the index.**

Pass: total system count per milestone tier, estimated implementation volume per tier (system count x average complexity), team size, stated project timeline.
...
Using the template at `plugins/codex-game-studios/assets/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).**
...
## Phase 6: Design Individual Systems (Handoff to $cgs-design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `$cgs-map-systems [system-name]`
- The user invokes `$cgs-map-systems next`
...
Use `ask the user directly or use available Codex UI question tools` for: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to $cgs-design-system

Once a system is selected, invoke the `$cgs-design-system [system-name]` skill.

The `$cgs-design-system` skill handles the full GDD authoring process:
...
- Runs `$cgs-design-review` when complete
- Updates the systems index

**Do not duplicate the $cgs-design-system workflow here.** This skill owns the systems
*index*; `$cgs-design-system` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `$cgs-design-system` completes, use `ask the user directly or use available Codex UI question tools`:
...
After the systems index is created (or after designing some systems), present next actions using `ask the user directly or use available Codex UI question tools`:

- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs -- run `$cgs-design-system [first-system-in-order]`
  - [B] Run `$cgs-gate-check systems-design` -- triggers the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates automatically for a formal director sign-off on the system set
  - [C] Stop here for this session

**The gate-check option ([B]) is worth highlighting**: running `$cgs-gate-check systems-design` triggers both the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates, catching scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual GDD is completed:
- "Run `$cgs-design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `$cgs-gate-check systems-design` when all MVP GDDs are complete"
...
2. **ask the user directly or use available Codex UI question tools** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `$cgs-design-system`
...
5. **Handoff**: Individual GDD authoring is owned by `$cgs-design-system`, which handles
...
> designing individual GDDs -- run `$cgs-map-systems next` to pick up where you left off.

---

## Recommended Next Steps

- Run `$cgs-design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `$cgs-map-systems next` to always pick the highest-priority undesigned system automatically
- Run `$cgs-design-review design/gdd/[system].md` in a fresh session after each GDD is authored
- Run `$cgs-gate-check pre-production` when all MVP GDDs are authored and reviewed
