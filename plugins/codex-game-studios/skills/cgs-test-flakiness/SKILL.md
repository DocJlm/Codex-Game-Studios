...
description: "Codex Game Studios skill adapted from original /test-flakiness. Use when the user asks for /test-flakiness, $cgs-test-flakiness, or this workflow. Detect non-deterministic (flaky) tests by reading CI run logs or test result history. Aggregates pass rates per test, identifies intermittent failures, recommends quarantine or fix, and maintains a flaky test registry. Best run during Polish phase or after multiple CI runs."
---

# CGS: test-flakiness

> Codex adaptation: this skill is migrated from the upstream `/test-flakiness` workflow. Invoke it as `$cgs-test-flakiness`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- After `$cgs-regression-suite` identifies quarantined tests that need diagnosis
...
**Modes:**
- `$cgs-test-flakiness [ci-log-path]` -- analyse a specific CI run log file
- `$cgs-test-flakiness scan` -- scan all available CI logs in `.github/` or
  standard log output directories
- `$cgs-test-flakiness registry` -- read existing regression-suite.md quarantine
...
> 3. Run `$cgs-test-flakiness registry` to review tests already flagged as flaky
