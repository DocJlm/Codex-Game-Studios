...
description: "Codex Game Studios skill adapted from original /balance-check. Use when the user asks for /balance-check, $cgs-balance-check, or this workflow. Analyzes game balance data files, formulas, and configuration to identify outliers, broken progressions, degenerate strategies, and economy imbalances. Use after modifying any balance-related data or design. Use when user says 'balance report', 'check game balance', 'run a balance check'."
---

# CGS: balance-check

> Codex adaptation: this skill is migrated from the upstream `/balance-check` workflow. Invoke it as `$cgs-balance-check`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
After presenting the report, use `ask the user directly or use available Codex UI question tools`:
...
- If the fix changes a tuning knob defined in a GDD or referenced by an ADR, remind the user:
  > "This value is defined in a design document. Run `$cgs-propagate-design-change [path]` on the affected GDD to find downstream impacts before committing."
...
- Confirm the file was written, then end with: "Re-run `$cgs-balance-check` after fixes to verify."

If [C]:
- Summarize open issues and end with: "Re-run `$cgs-balance-check` after fixes to verify."
