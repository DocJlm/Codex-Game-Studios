...
description: "Codex Game Studios skill adapted from original /create-architecture. Use when the user asks for /create-architecture, $cgs-create-architecture, or this workflow. Guided, section-by-section authoring of the master architecture document for the game. Reads all GDDs, the systems index, existing ADRs, and the engine reference library to produce a complete architecture blueprint before any code is written. Engine-version-aware: flags knowledge gaps and validates decisions against the pinned engine version."
---

# CGS: create-architecture

> Codex adaptation: this skill is migrated from the upstream `/create-architecture` workflow. Invoke it as `$cgs-create-architecture`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Distinct from `$cgs-architecture-decision`**: ADRs record individual point decisions.
...
> "No engine is configured. Run `$cgs-setup-engine` first. Architecture cannot be
...
3. `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` -- naming conventions, performance budgets,
...
```
  [ClassName.method()] -- Godot 4.6 (post-cutoff, HIGH risk)
...
- `$cgs-architecture-decision [title]` -> covers: TR-[id], TR-[id]

**Core Layer:**
- `$cgs-architecture-decision [title]` -> covers: TR-[id]
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `ask the user directly or use available Codex UI question tools`:
...
Apply gate **TD-ARCHITECTURE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 -- Spawn `lead-programmer` via Task using gate LP-FEASIBILITY (`plugins/codex-game-studios/references/studio-docs/director-gates.md`):**
...
Use `ask the user directly or use available Codex UI question tools` -- "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `ask the user directly or use available Codex UI question tools`:
...
**1. `$cgs-architecture-decision "[Title]"` -> ADR-[XXXX]**
[One sentence: what it defines and what it unblocks.]

**2. `$cgs-architecture-decision "[Title]"` -> ADR-[XXXX]**
[One sentence.]

**3. `$cgs-architecture-decision "[Title]"` -> ADR-[XXXX]**
[One sentence.]
...
> **Required before `$cgs-gate-check [stage]`:**
> - [ ] Accept ADRs: [list Proposed ADR IDs that must be Accepted]
> - [ ] Write ADRs: [list ADR IDs that must still be written]
> - [ ] Run `$cgs-test-setup` -- scaffolds `tests/unit/`, `tests/integration/`, CI workflow, and an example test file
> - [ ] Run `$cgs-ux-design` -- creates `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md`
>
> Run `$cgs-gate-check [stage]` when all boxes are checked.

If nothing is blocking, write instead:
> No blockers -- run `$cgs-gate-check [stage]` now.
...
5. **Use `ask the user directly or use available Codex UI question tools` for write approvals** -- plain text "May I?" is not
...
- Run `$cgs-architecture-decision [title]` for each required ADR listed in Phase 6 -- Foundation layer ADRs first
- Run `$cgs-architecture-review` -- bootstraps the Requirements Traceability Matrix and TR registry from the ADRs just written. Required before the Pre-Production gate.
- Run `$cgs-test-setup` to scaffold `tests/unit/`, `tests/integration/`, CI workflow, and an example test (required for gate-check)
- Run `$cgs-ux-design` to initialize `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md` (required for gate-check)
- Run `$cgs-create-control-manifest` once the required ADRs are written to produce the layer rules manifest
- Run `$cgs-gate-check pre-production` when all required ADRs, `$cgs-test-setup`, and `$cgs-ux-design` are complete
