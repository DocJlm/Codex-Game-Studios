---
name: cgs-story-done
description: "Codex Game Studios skill adapted from original /story-done. Use when the user asks for /story-done, $cgs-story-done, or this workflow. End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint."
---

# CGS: story-done

> Codex adaptation: this skill is migrated from the upstream `/story-done` workflow. Invoke it as `$cgs-story-done`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**If a file path is provided** (e.g., `$cgs-story-done production/epics/core/story-damage-calculator.md`):
...
3. If multiple in-progress stories are found, use `ask the user directly or use available Codex UI question tools`:
...
### Manual verification with confirmation (use `ask the user directly or use available Codex UI question tools`)
...
Batch up to 4 manual verification questions into a single `ask the user directly or use available Codex UI question tools` call:
...
1. Ask: is there a test -- unit, integration, or confirmed manual playtest -- that
   ...
   - **Manual confirmation**: if the criterion was verified via `ask the user directly or use available Codex UI question tools`
...
If none: flag as **ADVISORY** -- "No smoke check report found. Run `$cgs-smoke-check`."
...
2. **Manifest version staleness check**: Compare the `Manifest Version:` date
   ...
   - If the story's version is older -> flag as ADVISORY:
     ...
     is v[current-date]. New rules may apply. Run $cgs-story-readiness to check.`
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Task using gate **QL-TEST-COVERAGE** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).
...
- `lean` -> use `ask the user directly or use available Codex UI question tools` before proceeding:
  - Prompt: "Code review is skipped in lean mode. Did you run `$cgs-code-review` on the implemented files?"
  - Options:
    - `Yes -- $cgs-code-review passed or was approved with suggestions`
    - `No -- skipping code review for this story`
    - `No -- I'll run $cgs-code-review before the sprint close-out`
  - Record the answer in the completion notes (Phase 7). All three options proceed to Phase 6.
- `full` -> spawn as normal.

Spawn `lead-programmer` via Task using gate **LP-CODE-REVIEW** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `ask the user directly or use available Codex UI question tools`:
...
Use `ask the user directly or use available Codex UI question tools` before writing anything:
...
`production/session-state/active.md`:
    ...
    ## Session Extract -- $cgs-story-done [date]
    - Verdict: [COMPLETE / COMPLETE WITH NOTES / BLOCKED]
...
Run `$cgs-story-readiness [path]` to confirm a story is implementation-ready
before starting.
```
...
1. `$cgs-smoke-check sprint` -- verify the critical path still works end-to-end
2. `$cgs-team-qa sprint` -- full QA cycle: test case execution, bug triage, sign-off report
3. `$cgs-retrospective` -- capture what went well, what didn't, and action items for the next sprint
4. `$cgs-gate-check` -- advance to the next phase once QA approves (only if advancing a phase)
5. `$cgs-sprint-plan new` -- plan the next sprint, incorporating velocity data and retrospective action items

Do not run `$cgs-gate-check` until `$cgs-team-qa` returns APPROVED or APPROVED WITH CONDITIONS.
```
...
- Use `ask the user directly or use available Codex UI question tools` for the code review prompt and for batching manual
...
- Run `$cgs-story-readiness [next-story-path]` to validate the next story before starting implementation
- If all Must Have stories are complete: run `$cgs-smoke-check sprint` -> `$cgs-team-qa sprint` -> `$cgs-gate-check`
- If tech debt was logged: track it via `$cgs-tech-debt` to keep the register current
