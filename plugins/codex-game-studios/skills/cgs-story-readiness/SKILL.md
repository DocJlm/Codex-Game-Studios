...
description: "Codex Game Studios skill adapted from original /story-readiness. Use when the user asks for /story-readiness, $cgs-story-readiness, or this workflow. Validate that a story file is implementation-ready. Checks for embedded GDD requirements, ADR references, engine notes, clear acceptance criteria, and no open design questions. Produces READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I start on this story', 'is story X ready to implement'."
---

# CGS: story-readiness

> Codex adaptation: this skill is migrated from the upstream `/story-readiness` workflow. Invoke it as `$cgs-story-readiness`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
See `plugins/codex-game-studios/references/studio-docs/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via ask the user directly or use available Codex UI question tools)

- **Specific path** (e.g., `$cgs-story-readiness production/epics/combat/story-001-basic-attack.md`):
...
If no argument is given, use `ask the user directly or use available Codex UI question tools`:
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN`
  ...
  - If the ID does not exist in the registry -> NEEDS WORK: ID was not registered
    (story may predate registry, or registry needs an `$cgs-architecture-review` run).
...
Resolve these before the sprint begins or replan with `$cgs-sprint-plan update`.
```
...
`$cgs-create-stories`) handles writing.

**Redirect rules:**
- If a story file does not exist at all: "This story file is missing entirely.
  Run `$cgs-create-epics [layer]` then `$cgs-create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has
  ...
  `$cgs-quick-design [description]` to create a Quick Design Spec, then reference
...
Run `$cgs-story-readiness [path]` to validate before starting.
```
...
Spawn `qa-lead` via Task using gate **QL-STORY-READY** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).
...
- **GAPS [list]** -> surface the specific gaps to the user via `ask the user directly or use available Codex UI question tools`:
...
- Run `$cgs-dev-story [story-path]` to begin implementation once the story is READY
- Run `$cgs-story-readiness sprint` to check all stories in the current sprint at once
- Run `$cgs-create-stories [epic-slug]` if a story file is missing entirely
