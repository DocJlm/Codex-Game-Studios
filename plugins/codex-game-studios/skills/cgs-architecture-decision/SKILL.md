...
(e.g., `$cgs-architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` -- **BLOCKING if missing**: `$cgs-story-readiness` cannot check ADR acceptance
...
7. Suggest: "Run `$cgs-architecture-review` to re-validate coverage now that this ADR
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `$cgs-setup-engine` first, or tell me which engine you are using."
...
[N] Start writing GDDs -- run `$cgs-design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `$cgs-architecture-review` in a fresh session.
