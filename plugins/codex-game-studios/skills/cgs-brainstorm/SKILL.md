---
name: cgs-brainstorm
description: "Codex Game Studios skill adapted from original /brainstorm. Use when the user asks for /brainstorm, $cgs-brainstorm, or this workflow. Guided game concept ideation -- from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
---

# CGS: brainstorm

> Codex adaptation: this skill is migrated from the upstream `/brainstorm` workflow. Invoke it as `$cgs-brainstorm`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   See `plugins/codex-game-studios/references/studio-docs/director-gates.md` for the full check pattern.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask the user directly or use available Codex UI question tools`** at key decision points throughout brainstorming:
   ...
   `ask the user directly or use available Codex UI question tools` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text -- the user must be able to type specific game names freely.
  Do NOT put this in an ask the user directly or use available Codex UI question tools with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `ask the user directly or use available Codex UI question tools` for this -- constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `ask the user directly or use available Codex UI question tools` with these exact tab labels:
...
Present all three. Then use `ask the user directly or use available Codex UI question tools` to capture the selection.
...
Ask these as `ask the user directly or use available Codex UI question tools` calls -- derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `ask the user directly or use available Codex UI question tools`:
...
If the user selects B, C, or D, make the revision, then use `ask the user directly or use available Codex UI question tools` again:
...
- **`creative-director`** -- gate **CD-PILLARS** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** -- gate **AD-CONCEPT-VISUAL** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `ask the user directly or use available Codex UI question tools`:
...
- **Target platform**: Use `ask the user directly or use available Codex UI question tools` -- "What platforms are you targeting for this game?"
  Options: `PC (Steam / Epic)` / `Mobile (iOS / Android)` / `Console` / `Web / Browser` / `Multiple platforms`
  Record the answer -- it directly shapes the engine recommendation and will be passed to `$cgs-setup-engine`.
...
- **Engine experience**: Use `ask the user directly or use available Codex UI question tools` -- "Do you already have an engine you work in?"
  ...
  - If "No preference" -> tell them: "Run `$cgs-setup-engine` after this session -- it will walk you through the full decision based on your concept and platform target." Do not make a recommendation here.
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`plugins/codex-game-studios/references/studio-docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `plugins/codex-game-studios/assets/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `ask the user directly or use available Codex UI question tools` for write approval:
...
If [B]: ask which section to revise using `ask the user directly or use available Codex UI question tools` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else -- I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `ask the user directly or use available Codex UI question tools` -- "Ready to write the updated concept document?"
Options: `[A] Yes -- write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `plugins/codex-game-studios/assets/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
**Path A -- Design-First** (recommended if the concept is well-defined):
   1. "Run `$cgs-setup-engine` to configure the engine and populate version-aware reference docs"
   2. "Run `$cgs-art-bible` to create the visual identity specification -- do this BEFORE writing GDDs. **The art bible is required before the Technical Setup gate.** It gates asset production and shapes technical architecture decisions (rendering, VFX, UI systems)."
   3. "Use `$cgs-design-review design/gdd/game-concept.md` to validate concept completeness before going downstream"
   4. "Discuss vision with the `creative-director` agent for pillar refinement"
   5. "Decompose the concept into individual systems with `$cgs-map-systems` -- maps dependencies, assigns priorities, and creates the systems index"
   6. "Author per-system GDDs with `$cgs-design-system` -- guided, section-by-section GDD writing for each system identified in step 5"
   7. "Plan the technical architecture with `$cgs-create-architecture` -- produces the master architecture blueprint and Required ADR list"
   8. "Record key architectural decisions with `$cgs-architecture-decision (xN)` -- write one ADR per decision in the Required ADR list from `$cgs-create-architecture`"
   9. "Run `$cgs-architecture-review` -- bootstraps the TR registry and Requirements Traceability Matrix from your GDDs and ADRs (required before the Pre-Production gate)"
   10. "Validate readiness to advance with `$cgs-gate-check` -- phase gate before committing to production"

**Path B -- Prototype-First** (use if the core mechanic is unproven or the concept needs validation):
   1. "Run `$cgs-setup-engine` to configure the engine"
   2. "Run `$cgs-prototype [core-mechanic]` -- validate the core idea is fun before writing any GDDs (1-3 days throwaway code)"
   3. "If prototype PROCEEDS: run `$cgs-art-bible`, then continue with Path A steps 5-10 above, using prototype learnings to inform your GDDs"
   4. "If prototype PIVOTS: return to `$cgs-brainstorm` with the learnings and reshape the concept"
   5. "After full design and architecture, build the `$cgs-vertical-slice` to validate production readiness before committing to sprints"
...
1. `$cgs-setup-engine` -- configure the engine and populate version-aware reference docs
2. `$cgs-art-bible` -- establish visual identity before writing any GDDs
3. `$cgs-map-systems` -- decompose the concept into individual systems with dependencies
4. `$cgs-design-system [first-system]` -- author per-system GDDs in dependency order
5. `$cgs-create-architecture` -- produce the master architecture blueprint
6. `$cgs-architecture-review` -- bootstrap TR registry and Requirements Traceability Matrix
7. `$cgs-gate-check pre-production` -- validate readiness before committing to production
