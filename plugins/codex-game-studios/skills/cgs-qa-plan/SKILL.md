---
name: cgs-qa-plan
description: "Codex Game Studios skill adapted from original /qa-plan. Use when the user asks for /qa-plan, $cgs-qa-plan, or this workflow. Generate a QA test plan for a sprint or feature. Reads GDDs and story files, classifies stories by test type (Logic/Integration/Visual/UI), and produces a structured test plan covering automated tests required, manual test cases, smoke test scope, and playtest sign-off requirements. Run before sprint begins or when starting a major feature."
---

# CGS: qa-plan

> Codex adaptation: this skill is migrated from the upstream `/qa-plan` workflow. Invoke it as `$cgs-qa-plan`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Argument:** `$ARGUMENTS` (blank = ask user via ask the user directly or use available Codex UI question tools)
...
- **No argument** -- use `ask the user directly or use available Codex UI question tools`:
...
**Date**: [date]
**Generated by**: $cgs-qa-plan
**Scope**: [N stories across [N systems]]
**Engine**: [engine name from plugins/codex-game-studios/references/studio-docs/technical-preferences.md, or "Not configured"]
...
*Smoke tests are verified by the developer via `$cgs-smoke-check`. Reference this
...
- [ ] Smoke check passes (run `$cgs-smoke-check sprint` before QA hand-off)
- [ ] No regressions introduced
- [ ] Code reviewed (via `$cgs-code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (via `$cgs-story-done`)
````
...
then ask two questions together using `ask the user directly or use available Codex UI question tools`:
...
options:
  ...
  - "Also back-fill test case specs into each story file's ## QA Test Cases section (Recommended -- enables $cgs-dev-story and $cgs-code-review traceability)"
```
...
- Once all sprint stories are implemented, run `$cgs-smoke-check sprint` to gate QA hand-off -- not yet, only after implementation is complete
- For Logic/Integration stories, create the test files at the listed paths
  before marking stories done -- `$cgs-story-done` checks for them"
...
- Use `ask the user directly or use available Codex UI question tools` for scope selection when no argument is provided.
