---
name: cgs-ux-design
description: "Codex Game Studios skill adapted from original /ux-design. Use when the user asks for /ux-design, $cgs-ux-design, or this workflow. Guided, section-by-section UX spec authoring for a screen, flow, or HUD. Reads game concept, player journey, and relevant GDDs to provide context-aware design guidance. Produces ux-spec.md (per screen/flow) or hud-design.md using the studio templates."
---

# CGS: ux-design

> Codex adaptation: this skill is migrated from the upstream `/ux-design` workflow. Invoke it as `$cgs-ux-design`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**If no argument is provided**, do not fail -- ask instead. Use `ask the user directly or use available Codex UI question tools`:
...
- **Game concept**: Read `design/gdd/game-concept.md` -- if missing, warn:
  > "No game concept found. Run `$cgs-brainstorm` first to establish the game's
...
> "Player journey map not yet created. Template available at `plugins/codex-game-studios/assets/templates/player-journey.md`. Run `$cgs-ux-design` Phase 2b or create it manually to establish player context for this screen."
...
Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and extract the `## Input & Platform`
...
>
> (Run `$cgs-setup-engine` to save this permanently so you won't be asked again.)
...
2. **Questions**: Ask what is needed to draft this section. Use `ask the user directly or use available Codex UI question tools`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `ask the user directly or use available Codex UI question tools` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `ask the user directly or use available Codex UI question tools`:
...
7. **Write**: Use `ask the user directly or use available Codex UI question tools`: "May I write the [section name] section to `[filepath]`?"
...
- Use `ask the user directly or use available Codex UI question tools` to capture the choice:
...
- Use `ask the user directly or use available Codex UI question tools`: "Want an ASCII wireframe as part of this spec?"
...
> "Accessibility tier not yet defined -- consider WCAG-AA as a baseline. Run `$cgs-gate-check` to see whether this blocks any phase gates."
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `$cgs-story-done`.
...
Use `ask the user directly or use available Codex UI question tools` to confirm:
...
Use `ask the user directly or use available Codex UI question tools` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `ask the user directly or use available Codex UI question tools`:
...
> "This spec should be validated with `$cgs-ux-review` before it enters the
> implementation pipeline. The Pre-Production gate requires all key screen specs
> to have a review verdict."

Then use `ask the user directly or use available Codex UI question tools`:
- "Run `$cgs-ux-review [filename]` now, or do something else first?"
  - Options:
    - "Run `$cgs-ux-review` now -- validate this spec"
...
`$cgs-ux-review` on all completed specs before running `$cgs-gate-check pre-production`."
...
- Run `$cgs-ux-review [filename]` to validate this spec before it enters the implementation pipeline
- Run `$cgs-ux-design [next-screen]` to continue designing remaining screens or flows
- Run `$cgs-gate-check pre-production` once all key screens have approved UX specs
