...
description: "Codex Game Studios skill adapted from original /content-audit. Use when the user asks for /content-audit, $cgs-content-audit, or this workflow. Audit GDD-specified content counts against implemented content. Identifies what's planned vs built."
---

# CGS: content-audit

> Codex adaptation: this skill is migrated from the upstream `/content-audit` workflow. Invoke it as `$cgs-content-audit`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
to `$cgs-create-stories [epic-slug]` or `$cgs-quick-design` depending on the size of the gap.

### --summary mode

Print the Gap Table and Summary directly to conversation. Do not write a file.
End with: "Run `$cgs-content-audit` without `--summary` to write the full report."
...
- If any system is `NOT STARTED` and MVP-tagged -> "Run `$cgs-design-system [name]` to
  add missing content counts to the GDD before implementation begins."
- If total gap is >50% -> "Run `$cgs-sprint-plan` to allocate content work across upcoming sprints."
- If backlog stories are needed -> "Run `$cgs-create-stories [epic-slug]` for each HIGH PRIORITY gap."
- If `--summary` was used -> "Run `$cgs-content-audit` (no flag) to write the full report to `docs/`."
