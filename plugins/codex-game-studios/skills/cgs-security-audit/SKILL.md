...
description: "Codex Game Studios skill adapted from original /security-audit. Use when the user asks for /security-audit, $cgs-security-audit, or this workflow. Audit the game for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure, and input validation gaps. Produces a prioritised security report with remediation guidance. Run before any public release or multiplayer launch."
---
...
Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` to determine:
...
**Audited by**: security-engineer via $cgs-security-audit
...
Run `$cgs-security-audit` again after remediating any CRITICAL or HIGH findings.
...
After remediating findings, re-run: `$cgs-security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `$cgs-gate-check release`.

If CRITICAL findings exist:
> " CRITICAL security findings must be resolved before any public release. Do not proceed to `$cgs-launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> " No blocking security findings. Report written to `production/security/`. Include this path when running `$cgs-gate-check release`."
