...
description: "Codex Game Studios skill adapted from original /playtest-report. Use when the user asks for /playtest-report, $cgs-playtest-report, or this workflow. Generates a structured playtest report template or analyzes existing playtest notes into a structured format. Use this to standardize playtest feedback collection and analysis."
---
...
- **Design changes:** "Run `$cgs-propagate-design-change [path]` on the affected design document to find downstream impacts before making changes."
- **Balance adjustments:** "Run `$cgs-balance-check [system]` to verify the full balance picture before tuning values."
- **Bugs:** "Use `$cgs-bug-report` to formally track these."
...
- After addressing design changes: re-run `$cgs-design-review` on the updated GDD.
- After fixing bugs: re-run `$cgs-bug-triage` to update priorities.
