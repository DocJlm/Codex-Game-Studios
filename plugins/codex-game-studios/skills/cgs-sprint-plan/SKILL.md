...
description: "Codex Game Studios skill adapted from original /sprint-plan. Use when the user asks for /sprint-plan, $cgs-sprint-plan, or this workflow. Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs."
---
...
- [ ] Smoke check passed (`$cgs-smoke-check sprint`)
- [ ] QA sign-off report: APPROVED or APPROVED WITH CONDITIONS (`$cgs-team-qa sprint`)
...
`$cgs-sprint-status`, `$cgs-story-done`, and `$cgs-help` without markdown parsing.
...
```yaml
# Auto-generated by $cgs-sprint-plan. Updated by $cgs-story-done and $cgs-dev-story.
# DO NOT edit manually -- use $cgs-story-done to update story status.
#
...
> **Scope check:** If this sprint includes stories added beyond the original epic scope, run `$cgs-scope-check [epic]` to detect scope creep before implementation begins.
...
If [A]: close with "Sprint plan written. Run `$cgs-qa-plan sprint` next -- then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
>  **No QA Plan**: This sprint was started without a QA plan. Run `$cgs-qa-plan sprint`
...
- `$cgs-qa-plan sprint` -- **required before implementation begins** -- defines test cases per story so developers implement against QA specs, not a blank slate
- `$cgs-story-readiness [story-file]` -- validate a story is ready before starting it
- `$cgs-dev-story [story-file]` -- begin implementing the first story
- `$cgs-sprint-status` -- check progress mid-sprint
- `$cgs-scope-check [epic]` -- verify no scope creep before implementation begins
