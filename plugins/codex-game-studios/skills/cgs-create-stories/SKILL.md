...
description: "Codex Game Studios skill adapted from original /create-stories. Use when the user asks for /create-stories, $cgs-create-stories, or this workflow. Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after $cgs-create-epics for each epic."
---

# CGS: create-stories

> Codex adaptation: this skill is migrated from the upstream `/create-stories` workflow. Invoke it as `$cgs-create-stories`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Previous step:** `$cgs-create-epics [system]`
**Next step after stories exist:** `$cgs-story-readiness [story-path]` then `$cgs-dev-story [story-path]`
...
in this skill -- apply the check pattern from `plugins/codex-game-studios/references/studio-docs/director-gates.md`
before every gate invocation.

- `$cgs-create-stories [epic-slug]` -- e.g. `$cgs-create-stories combat`
- `$cgs-create-stories production/epics/combat/EPIC.md` -- full path also accepted
...
> Check the filename in the epic's Governing ADRs list, or run `$cgs-architecture-decision`
...
The type determines what test evidence is required before `$cgs-story-done` can close the story.
...
- **Governing ADR**: which ADR governs how to implement this?
  ...
  - `Status: Proposed` -> set story `Status: Blocked` with note: "BLOCKED: ADR-NNNN is Proposed -- run `$cgs-architecture-decision` to advance it"
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via Task using gate **QL-STORY-READY** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).
...
- If "Skip": leave `## QA Test Cases` with a placeholder: `*Test cases not yet defined -- run $cgs-qa-plan to generate them.*`
...
> **Last Updated**: [set by $cgs-dev-story when implementation begins]
...
Use `ask the user directly or use available Codex UI question tools` to close with context-aware next steps:

Check:
- Are there other epics in `production/epics/` without stories yet? List them.
- Is this the last epic? If so, include `$cgs-sprint-plan` as an option.

Widget:
- Prompt: "[N] stories written to `production/epics/[epic-slug]/`. What next?"
- Options (include all that apply):
  - `[A] Start implementing -- run $cgs-story-readiness [first-story-path]` (Recommended)
  - `[B] Create stories for [next-epic-slug] -- run $cgs-create-stories [slug]` (only if other epics have no stories yet)
  - `[C] Plan the sprint -- run $cgs-sprint-plan new` (only if all epics have stories)
...
- **Verdict: COMPLETE** -- [N] stories written to `production/epics/[epic-slug]/`. Run `$cgs-story-readiness` -> `$cgs-dev-story` to begin implementation.
