...
description: "Codex Game Studios skill adapted from original /review-all-gdds. Use when the user asks for /review-all-gdds, $cgs-review-all-gdds, or this workflow. Holistic cross-GDD consistency and game design review. Reads all system GDDs simultaneously and checks for contradictions between them, stale references, ownership conflicts, formula incompatibilities, and game design theory violations (dominant strategies, economic imbalance, cognitive overload, pillar drift). Run after all MVP GDDs are written, before architecture begins."
---
...
**This is distinct from `$cgs-design-review`**, which reviews one GDD for internal
...
- Before `$cgs-create-architecture` begins (architecture built on inconsistent GDDs
...
Run `$cgs-consistency-check` after this review to populate the registry."
...
> then re-run `$cgs-review-all-gdds`."
...
```
  Difficulty Curve Mismatch
combat.md: Enemy health scales exponentially with area (x2 per area)
progression.md: Player damage scales linearly with level (+10% per level)
-> By area 5, enemies have 32x base health; player deals ~1.5x base damage.
...
-> progression.md (again): quest XP added -> triggers SECOND level-up in same frame
    Data flow issue: quest.md awards XP without checking if a level-up
...
#### Info
i  [Scenario name] -- [Systems involved]
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract -- $cgs-review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
...
- Are any GDDs in the "Flagged for Revision" table? -> offer $cgs-design-review option for each
- Read systems-index.md for the next system with Status: Not Started -> offer $cgs-design-system option
- Is the verdict PASS or CONCERNS? -> offer $cgs-gate-check or $cgs-create-architecture
...
- `[_] Run $cgs-design-review [flagged-gdd-path] -- address flagged warnings` (one per flagged GDD, if any)
- `[_] Run $cgs-design-system [next-system] -- next in design order` (always include, name the actual system)
- `[_] Run $cgs-create-architecture -- begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run $cgs-gate-check -- validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
