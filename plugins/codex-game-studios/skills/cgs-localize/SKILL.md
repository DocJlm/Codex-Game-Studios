...
### Issues
- [N] hardcoded strings found in source code (run $cgs-localize scan)
...
Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` to determine the engine. Then check:
...
[ ] $cgs-localize scan shows zero hardcoded strings
[ ] $cgs-localize validate shows no placeholder mismatches in source (en)
...
## Post-Freeze Changes
[Any strings added or modified after freeze are listed here automatically by $cgs-localize extract]
```
...
> " String freeze is active. [N] new/modified strings have been added. These are freeze violations. Notify your localization vendor before proceeding."
...
```
$cgs-localize scan            -> find hardcoded strings
$cgs-localize extract         -> build string table
$cgs-localize freeze          -> lock source before sending to translators
$cgs-localize brief           -> generate translator briefing document
[Send to translators]
$cgs-localize validate        -> check returned translations
$cgs-localize cultural-review -> flag culturally sensitive content
$cgs-localize rtl-check       -> if shipping Arabic / Hebrew / Persian
$cgs-localize vo-pipeline     -> if shipping dubbed VO
$cgs-localize qa              -> full localization QA pass
```

After `qa` returns PASS for all shipping locales, include the QA report path when running `$cgs-gate-check release`.
