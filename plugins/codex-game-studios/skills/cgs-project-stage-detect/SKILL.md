...
description: "Codex Game Studios skill adapted from original /project-stage-detect. Use when the user asks for /project-stage-detect, $cgs-project-stage-detect, or this workflow. Automatically analyze project state, detect stage, identify gaps, and recommend next steps based on existing artifacts. Use when user asks 'where are we in development', 'what stage are we in', 'full project audit'."
---

# CGS: project-stage-detect

> Codex adaptation: this skill is migrated from the upstream `/project-stage-detect` workflow. Invoke it as `$cgs-project-stage-detect`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
if it exists, use its value (explicit override from `$cgs-gate-check`). Otherwise,
...
| **Polish** | Explicit only (set by `$cgs-gate-check` Production -> Polish gate) |
| **Release** | Explicit only (set by `$cgs-gate-check` Polish -> Release gate) |
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `$cgs-map-systems`?"
...
Use template: `plugins/codex-game-studios/assets/templates/project-stage-report.md`
...
If user provided a role argument (e.g., `$cgs-project-stage-detect programmer`):
...
$cgs-project-stage-detect programmer

# Designer-focused analysis
$cgs-project-stage-detect designer
```
...
- **Concept exists but no systems index?** -> `$cgs-map-systems` to decompose into systems
- **Missing design docs?** -> `$cgs-reverse-document design src/[system]`
- **Missing architecture docs?** -> `$cgs-architecture-decision` or `$cgs-reverse-document architecture`
- **Prototypes need documentation?** -> `$cgs-reverse-document concept prototypes/[name]`
- **No sprint plan?** -> `$cgs-sprint-plan`
- **Approaching milestone?** -> `$cgs-milestone-review`
