---
name: cgs-bug-triage
description: "Codex Game Studios skill adapted from original /bug-triage. Use when the user asks for /bug-triage, $cgs-bug-triage, or this workflow. Read all open bugs in production/qa/bugs/, re-evaluate priority vs. severity, assign to sprints, surface systemic trends, and produce a triage report. Run at sprint start or when the bug count grows enough to need re-prioritization."
---
...
- After `$cgs-team-qa` completes and new bugs have been filed
...
**Modes:**
- `$cgs-bug-triage sprint` -- triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `$cgs-bug-triage full` -- full triage of all bugs regardless of sprint scope
- `$cgs-bug-triage trend` -- trend analysis only (no assignment); read-only report
...
Read `plugins/codex-game-studios/references/studio-docs/coding-standards.md` for severity/priority definitions if they
...
>  [N] high-severity bugs have been open for more than 2 sprints without
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `$cgs-sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found -- consider re-opening the
  affected stories in sprint tracking and running `$cgs-smoke-check` to re-gate."
