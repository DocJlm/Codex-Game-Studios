---
name: cgs-skill-test
description: "Codex Game Studios skill adapted from original /skill-test. Use when the user asks for /skill-test, $cgs-skill-test, or this workflow. Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report)."
---
...
**FAIL** if `tool expectations` includes `Write` or `Edit` but no ask-before-write language is found.

### Check 5 -- Next-Step Handoff
The skill must end with a recommended next action or follow-up path. Look for:
- A final section mentioning another skill (e.g., `$cgs-story-done`, `$cgs-gate-check`)
...
`usage note` must be non-empty. If the skill body mentions multiple modes
...
- Missing skill: "Skill '[name]' not found in `plugins/codex-game-studios/skills/`."
- Missing spec path in catalog: "No spec path set for '[name]' in catalog.yaml."
- Spec file not found at path: "Spec file missing at [path]. Run `$cgs-skill-test audit`
...
Offer: "Would you like to run `$cgs-skill-test static all` to check structural
compliance across all skills? `$cgs-skill-test category all` to run category rubric
checks? Or `$cgs-skill-test spec [name]` to run a specific behavioral test?"
...
- After `static [name]`: "Run `$cgs-skill-test spec [name]` to validate behavioral
