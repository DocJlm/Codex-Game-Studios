...
description: "Codex Game Studios skill adapted from original /sprint-status. Use when the user asks for /sprint-status, $cgs-sprint-status, or this workflow. Fast sprint status check. Reads the current sprint plan, scans story files for status, and produces a concise progress snapshot with burndown assessment and emerging risks. Run at any time during a sprint for quick situational awareness. Use when user asks 'how is the sprint going', 'sprint update', 'show sprint progress'."
---

# CGS: sprint-status

> Codex adaptation: this skill is migrated from the upstream `/sprint-status` workflow. Invoke it as `$cgs-sprint-status`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
`$cgs-sprint-plan update` or `$cgs-milestone-review`.
...
- If an argument is given (e.g., `$cgs-sprint-status 3`), search
...
- If `production/sprints/` does not exist or is empty, report: "No sprint
  files found. Start a sprint with `$cgs-sprint-plan new`." Then stop.
...
" No `sprint-status.yaml` found -- status inferred from markdown. Run `$cgs-sprint-plan update` to generate one."
...
time remaining. Recommend replanning with `$cgs-sprint-plan update`.
```
...
Run `$cgs-story-readiness sprint` to validate story file coverage.
```
...
- It does not propose scope cuts (that is `$cgs-sprint-plan update`)
- It makes at most one recommendation per run

For more detail on a specific story, the user can read the story file directly
or run `$cgs-story-readiness [path]`.

For sprint replanning, use `$cgs-sprint-plan update`.
For end-of-sprint retrospective, use `$cgs-retrospective`.
