...
**When to run:** After `$cgs-architecture-review` passes and ADRs are in Accepted
...
> **Status**: [Active -- regenerate with `$cgs-create-control-manifest update` when ADRs change]

`Manifest Version` is the date this manifest was generated. Story files embed
this date when created. `$cgs-story-readiness` compares a story's embedded version
...
- If epics/stories don't exist yet: "Run `$cgs-create-epics layer: foundation` then `$cgs-create-stories [epic-slug]` -- programmers
