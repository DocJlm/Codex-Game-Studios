...
description: "Codex Game Studios skill adapted from original /setup-engine. Use when the user asks for /setup-engine, $cgs-setup-engine, or this workflow. Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data."
---

# CGS: setup-engine

> Codex adaptation: this skill is migrated from the upstream `/setup-engine` workflow. Invoke it as `$cgs-setup-engine`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
- **Full spec**: `$cgs-setup-engine godot 4.6` -- engine and version provided
- **Engine only**: `$cgs-setup-engine unity` -- engine provided, version will be looked up
- **No args**: `$cgs-setup-engine` -- fully guided mode (engine recommendation + version)
- **Refresh**: `$cgs-setup-engine refresh` -- update reference docs (see Section 10)
- **Upgrade**: `$cgs-setup-engine upgrade [old-version] [new-version]` -- migrate to a new engine version (see Section 11)
...
- Read `design/gdd/game-concept.md` if it exists -- extract genre, scope, platform
  targets, art style, team size, and any engine recommendation from `$cgs-brainstorm`
- If no concept exists, inform the user:
  > "No game concept found. Consider running `$cgs-brainstorm` first to discover what
...
**Question 1 -- Prior experience** (ask this first, always, via `ask the user directly or use available Codex UI question tools`):
...
**Question 2 -- Target platform** (ask this second, always, via `ask the user directly or use available Codex UI question tools` -- platform eliminates or heavily weights engines before any other factor):
...
5. Use `ask the user directly or use available Codex UI question tools` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `ask the user directly or use available Codex UI question tools` with concept-specific deep-dive topics. Always generate these options from the user's actual concept -- do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `ask the user directly or use available Codex UI question tools`:
...
Run `$cgs-setup-engine refresh` to populate full reference docs at any time.
```
...
If invoked as `$cgs-setup-engine refresh`:
...
If invoked as `$cgs-setup-engine upgrade [old-version] [new-version]`:
...
2. Run $cgs-setup-engine refresh after upgrading the actual engine binary to
   verify no new deprecations were missed
3. Run $cgs-architecture-review -- the engine upgrade may invalidate ADRs that
   reference specific APIs or engine capabilities
4. If any ADRs are invalidated, run $cgs-propagate-design-change to update
   downstream stories
```
...
2. [If from $cgs-brainstorm] Run $cgs-map-systems to decompose your concept into individual systems
3. [If from $cgs-brainstorm] Run $cgs-design-system to author per-system GDDs (guided, section-by-section)
4. [If from $cgs-brainstorm] Run $cgs-prototype [core-mechanic] to validate the core idea before writing GDDs
5. [If fresh start] Run $cgs-brainstorm to discover your game concept
6. Create your first milestone: $cgs-sprint-plan new
```
