...
description: "Codex Game Studios skill adapted from original /vertical-slice. Use when the user asks for /vertical-slice, $cgs-vertical-slice, or this workflow. Pre-Production validation -- build a production-quality end-to-end build to confirm the full game loop is achievable before committing to Production. Run after GDDs, architecture, and UX specs are complete. Produces a PROCEED/PIVOT/KILL verdict that gates the Pre-Production to Production transition."
---
...
whether the core idea is worth designing, run `$cgs-prototype` (concept prototype) instead.
...
- Naming conventions from `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`
...
Read `plugins/codex-game-studios/assets/templates/vertical-slice-report.md` to get the report structure.
...
Recommended next steps:
- `$cgs-create-epics layer:foundation` -- plan Foundation layer epics
- `$cgs-create-epics layer:core` -- plan Core layer epics
- `$cgs-create-stories [epic-slug]` -- break each epic into implementable stories
- `$cgs-sprint-plan` -- plan the first sprint using velocity data from the slice
- `$cgs-gate-check pre-production` -- formally advance the stage to Production

**Playtest note:** `$cgs-gate-check` will look for documented playtest evidence.
...
differently. When `$cgs-vertical-slice` is next run after a PIVOT, check the
`prototypes/` directory for a `PIVOT-NOTE.md` -- use it to frame the new validation
question and inform scope decisions.

- Revise affected GDDs with `$cgs-design-system [mechanic]`
- Address architecture issues via `$cgs-architecture-decision`
- Then re-run `$cgs-vertical-slice` to validate the revised direction
...
- Return to `$cgs-brainstorm` with what you learned
- Or run `$cgs-prototype [new-concept]` to test a new direction cheaply first
