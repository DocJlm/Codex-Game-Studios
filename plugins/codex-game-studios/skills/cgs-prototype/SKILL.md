---
name: cgs-prototype
description: "Codex Game Studios skill adapted from original /prototype. Use when the user asks for /prototype, $cgs-prototype, or this workflow. Concept prototype -- validate the core idea is worth designing before writing GDDs. Run right after $cgs-brainstorm and $cgs-setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict."
---
...
**Default use** -- run right after `$cgs-brainstorm` and `$cgs-setup-engine`, before writing
...
before committing to Production, run `$cgs-vertical-slice` instead.
...
[answer]." Suggest next step: `$cgs-map-systems` or `$cgs-design-system [mechanic]`.
...
Read `AGENTS.md` and `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` for the engine and
language in use.
...
Read `plugins/codex-game-studios/assets/templates/prototype-report.md` to get the report structure.
...
1. `$cgs-design-review design/gdd/game-concept.md` -- validate the concept doc against what the prototype revealed
2. `$cgs-gate-check` -- confirm readiness to advance to Systems Design
3. `$cgs-art-bible` -- define visual identity (optional but worth doing before GDDs)
4. `$cgs-map-systems` -- decompose the concept into all game systems
5. `$cgs-design-system [mechanic]` -- GDD for each MVP system; use prototype learnings
   in the Tuning Knobs and Formulas sections
6. `$cgs-review-all-gdds` -- cross-system consistency check
...
the revised hypothesis for the next prototype. When `$cgs-prototype` is next run, check
...
- Run `$cgs-prototype [revised-concept]` to test the adjusted direction
- Or `$cgs-brainstorm [hint]` if the concept needs more fundamental rethinking
...
- Run `$cgs-brainstorm open` or `$cgs-brainstorm [new-hint]` to explore a different concept
