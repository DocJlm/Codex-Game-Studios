---
name: cgs-help
description: "Codex Game Studios skill adapted from original /help. Use when the user asks for /help, $cgs-help, or this workflow. Analyzes what is done and the users query and offers advice on what to do next. Use if user says what should I do next or what do I do now or I'm stuck or I don't know what to do"
---

# CGS: help

> Codex adaptation: this skill is migrated from the upstream `/help` workflow. Invoke it as `$cgs-help`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
gap analysis, use `$cgs-project-stage-detect`.

---

## Step 1: Read the Catalog

Read `plugins/codex-game-studios/references/studio-docs/workflow-catalog.yaml`. This is the authoritative list of all
...
After reading the catalog, Glob `plugins/codex-game-studios/skills/*/SKILL.md` to get the full list
...
---
Approaching **[next phase]** gate -> run `$cgs-gate-check` when ready.
```
...
- If all required steps in the current phase are complete (or nearly complete),
  add: "You're close to the **[Current] -> [Next]** gate. Run `$cgs-gate-check` when ready."
...
---
Need more detail?
- `$cgs-project-stage-detect` -- full gap analysis with all missing artifacts listed
- `$cgs-gate-check` -- formal readiness check for your next phase
- `$cgs-start` -- re-orient from scratch
```
