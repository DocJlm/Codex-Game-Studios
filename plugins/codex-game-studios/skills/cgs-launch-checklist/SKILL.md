...
description: "Codex Game Studios skill adapted from original /launch-checklist. Use when the user asks for /launch-checklist, $cgs-launch-checklist, or this workflow. Complete launch readiness validation covering every department: code, content, store, marketing, community, infrastructure, legal, and go/no-go sign-offs."
---

# CGS: launch-checklist

> Codex adaptation: this skill is migrated from the upstream `/launch-checklist` workflow. Invoke it as `$cgs-launch-checklist`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `$cgs-launch-checklist`. Do not auto-invoke based on context matching.
...
- Run `$cgs-gate-check` to get a formal PASS/CONCERNS/FAIL verdict before launch.
- Coordinate sign-offs via `$cgs-team-release`.
