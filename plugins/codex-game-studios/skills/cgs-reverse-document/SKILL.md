...
description: "Codex Game Studios skill adapted from original /reverse-document. Use when the user asks for /reverse-document, $cgs-reverse-document, or this workflow. Generate design or architecture documents from existing implementation. Works backwards from code/prototypes to create missing planning docs."
---

# CGS: reverse-document

> Codex adaptation: this skill is migrated from the upstream `/reverse-document` workflow. Invoke it as `$cgs-reverse-document`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Format**: `$cgs-reverse-document <type> <path>`
...
**Examples**:
```bash
$cgs-reverse-document design src/gameplay/magic-system
$cgs-reverse-document architecture src/core/entity-component
$cgs-reverse-document concept prototypes/vehicle-combat
```
...
1. Run $cgs-balance-check on combat formulas (exponential scaling concern)
...
```
User: $cgs-reverse-document design src/gameplay/[system]
...
Agent:  Written to design/gdd/[system-name].md
       ...
       - Run $cgs-balance-check to validate [curve]
