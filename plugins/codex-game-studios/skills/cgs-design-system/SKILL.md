...
description: "Codex Game Studios skill adapted from original /design-system. Use when the user asks for /design-system, $cgs-design-system, or this workflow. Guided, section-by-section GDD authoring for a single game system. Gathers context from existing docs, walks through each required section collaboratively, cross-references dependencies, and writes incrementally to file."
---

# CGS: design-system

> Codex adaptation: this skill is migrated from the upstream `/design-system` workflow. Invoke it as `$cgs-design-system`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask the user directly or use available Codex UI question tools`:
...
3. If no systems index exists, fail with:
   > "Usage: `$cgs-design-system <system-name>` -- e.g., `$cgs-design-system movement`
   > Or to fill gaps in an existing GDD: `$cgs-design-system retrofit design/gdd/[system-name].md`
   > No systems index found. Run `$cgs-map-systems` first to map your systems and get the design order."
...
- **Game concept**: Read `design/gdd/game-concept.md` -- fail if missing:
  > "No game concept found. Run `$cgs-brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` -- fail if missing:
  > "No systems index found. Run `$cgs-map-systems` first to map your systems."
...
- Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` to identify the engine and version
...
> `$cgs-setup-engine` before moving to architecture if you haven't already."
...
Use the template structure from `plugins/codex-game-studios/assets/templates/game-design-document.md`:
...
> "Verdict: **BLOCKED** -- skeleton creation declined. The design session cannot proceed without the skeleton file, as all subsequent phases use it as the base. Re-run `$cgs-design-system [system]` when ready to create the file."
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask the user directly or use available Codex UI question tools` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask the user directly or use available Codex UI question tools` to capture the decision.
...
6. **Approval**: Immediately after the draft -- in the SAME response -- use
   `ask the user directly or use available Codex UI question tools`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `ask the user directly or use available Codex UI question tools` with a multi-tab widget:
...
in `$cgs-architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
**Framing question (ask BEFORE drafting)**: Use `ask the user directly or use available Codex UI question tools`:
...
- Surface any disagreements between agents to the user via `ask the user directly or use available Codex UI question tools`
...
- Present the specialists' proposals to the user for review via `ask the user directly or use available Codex UI question tools`
...
> " **Asset Spec** -- Visual/Audio requirements are defined. After the art bible is approved, run `$cgs-asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> (Pre-Production), run `$cgs-ux-design` to create a UX spec for each screen or
...
Before finalizing the GDD, spawn `creative-director` via Task using gate **CD-GDD-ALIGN** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).
...
> `$cgs-design-review design/gdd/[system-name].md`
>
> **Never run `$cgs-design-review` in the same session as `$cgs-design-system`.** The reviewing
...
**NEVER offer to run `$cgs-design-review` inline.** Always direct the user to a fresh window.
...
- "What's next?"
  - Options:
    - "Run `$cgs-consistency-check` -- verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" -- if undesigned systems remain
    - "Fix review findings" -- if design-review flagged issues
    - "Stop here for this session"
    - "Run `$cgs-gate-check`" -- if enough MVP systems are designed
...
- The main session presents the agent's output to the user via `ask the user directly or use available Codex UI question tools`
- The user decides; the main session writes to file
...
> open a fresh Codex session and run `$cgs-design-system [system-name]` -- it will
...
- Run `$cgs-design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `$cgs-consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `$cgs-map-systems next` to move to the next highest-priority undesigned system
