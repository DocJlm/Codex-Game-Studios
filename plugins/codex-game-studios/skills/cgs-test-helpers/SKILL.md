...
description: "Codex Game Studios skill adapted from original /test-helpers. Use when the user asks for /test-helpers, $cgs-test-helpers, or this workflow. Generate engine-specific test helper libraries for the project's test suite. Reads existing test patterns and produces tests/helpers/ with assertion utilities, factory functions, and mock objects tailored to the project's systems. Reduces boilerplate in new test files."
---

# CGS: test-helpers

> Codex adaptation: this skill is migrated from the upstream `/test-helpers` workflow. Invoke it as `$cgs-test-helpers`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**When to run:**
- After `$cgs-test-setup` scaffolds the framework (first time)
...
**Modes:**
- `$cgs-test-helpers [system-name]` -- generate helpers for a specific system
  (e.g., `$cgs-test-helpers combat`)
- `$cgs-test-helpers all` -- generate helpers for all systems with test files
- `$cgs-test-helpers scaffold` -- generate only the base helper library (no
...
Read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and extract:
- `Engine:` value
- `Language:` value
- `Framework:` from the Testing section

If engine is not configured: "Engine not configured. Run `$cgs-setup-engine` first."
...
## Generated by $cgs-test-helpers combat on [date].
...
- Run `$cgs-test-setup` if the test framework has not been scaffolded yet.
- Use `$cgs-dev-story` to implement stories -- helpers reduce boilerplate in new test files.
- Run `$cgs-skill-test` to validate other skills that may need helper coverage.
