---
name: cgs-hotfix
description: "Codex Game Studios skill adapted from original /hotfix. Use when the user asks for /hotfix, $cgs-hotfix, or this workflow. Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly."
---
...
Apply the verdict:
- **Smoke check sufficient** -- run `$cgs-smoke-check` against the hotfix build. If PASS, proceed to Phase 6.
- **Targeted QA pass required** -- run `$cgs-team-qa [affected-system]` scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** -- S1 fixes that touch core systems may require a full `$cgs-team-qa sprint`. This delays deployment but prevents a bad patch.
...
Next: $cgs-bug-report verify [BUG-ID] after deploy to confirm resolution
```
...
After deploying, run `$cgs-bug-report verify [BUG-ID]` to confirm the fix resolved the issue in the deployed build.

If VERIFIED FIXED: run `$cgs-bug-report close [BUG-ID]` to formally close it.
...
- Options:
  ...
  - `[B] Run $cgs-patch-notes to document this hotfix`
  - `[C] Stop here`
