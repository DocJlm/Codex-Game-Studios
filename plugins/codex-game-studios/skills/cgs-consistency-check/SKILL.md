...
**This skill is the write-time safety net.** It catches what `$cgs-design-system`'s
per-section checks may have missed and what `$cgs-review-all-gdds`'s holistic review
catches too late.

**When to run:**
- After writing each new GDD (before moving to the next system)
- Before `$cgs-review-all-gdds` (so that skill starts with a clean baseline)
- Before `$cgs-create-architecture` (inconsistencies poison downstream ADRs)
- On demand: `$cgs-consistency-check entity:[name]` to check one entity specifically
...
> "Entity registry is empty. Run `$cgs-design-system` to write GDDs -- the registry
...
This is the core optimization: instead of reading 10 GDDs x 400 lines each
(4,000 lines), you grep 50 entity names x 10 GDDs (50 targeted searches,
...
- ** STALE REGISTRY** -- source GDD value changed but registry not updated.
  Registry needs updating; other GDDs may be correct already.
- **i UNVERIFIABLE** -- entity mentioned but no comparable attribute stated.
...
i [gdd].md mentions [entity_name] but states no comparable attributes.
...
```markdown
### [YYYY-MM-DD] -- $cgs-consistency-check --  CONFLICT
...
<!-- Auto-maintained by $cgs-consistency-check. Do not edit manually. -->
...
- **Options**:
  ...
  - `[C] Run $cgs-design-review on the most conflicted GDD`
  - `[D] Stop here`
...
- **If PASS**: Run `$cgs-review-all-gdds` for holistic design-theory review, or
  `$cgs-create-architecture` if all MVP GDDs are complete.
- **If CONFLICTS FOUND**: Fix the flagged GDDs, then re-run
  `$cgs-consistency-check` to confirm resolution.
- **If STALE REGISTRY**: Update the registry (Phase 6), then re-run to verify.
- Run `$cgs-consistency-check` after writing each new GDD to catch issues early,
