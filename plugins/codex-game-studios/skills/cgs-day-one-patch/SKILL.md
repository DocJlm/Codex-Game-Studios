...
If [C]: output "No day-one patch required. Proceed to `$cgs-launch-checklist`." Stop.
...
- **Targeted smoke check** -- run `$cgs-smoke-check [affected-systems]`
...
1. Run `$cgs-patch-notes` to generate the player-facing version of the patch notes
2. Run `$cgs-bug-report verify [BUG-ID]` for each fixed bug after the patch is live
3. Run `$cgs-bug-report close [BUG-ID]` for each verified fix
4. Schedule a post-launch review 48-72 hours after launch using `$cgs-retrospective launch`
...
- Options:
  ...
  - `[B] Run $cgs-bug-report to log any issues found post-deploy`
  - `[C] Stop here`
...
- **Player communication is part of the patch** -- `$cgs-patch-notes` is a required output, not optional
