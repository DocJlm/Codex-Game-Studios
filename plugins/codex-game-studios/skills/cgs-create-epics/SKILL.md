...
description: "Codex Game Studios skill adapted from original /create-epics. Use when the user asks for /create-epics, $cgs-create-epics, or this workflow. Translate approved GDDs + architecture into epics -- one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories -- run $cgs-create-stories [epic-slug] after each epic is created."
---

# CGS: create-epics

> Codex adaptation: this skill is migrated from the upstream `/create-epics` workflow. Invoke it as `$cgs-create-epics`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**Next step after each epic:** `$cgs-create-stories [epic-slug]`

**When to run:** After `$cgs-create-control-manifest` and `$cgs-architecture-review` pass.
...
**Modes:**
- `$cgs-create-epics all` -- process all systems in layer order
- `$cgs-create-epics layer: foundation` -- Foundation layer only
- `$cgs-create-epics layer: core` -- Core layer only
- `$cgs-create-epics layer: feature` -- Feature layer only
- `$cgs-create-epics layer: presentation` -- Presentation layer only
- `$cgs-create-epics [system-name]` -- one specific system
...
> " [N] requirements in [system] have no ADR. The epic can be created, but
> stories for these requirements will be marked Blocked until ADRs exist.
> Run `$cgs-architecture-decision` first, or proceed with placeholders."
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via Task using gate **PR-EPIC** (`plugins/codex-game-studios/references/studio-docs/director-gates.md`).
...
> **Status**: Ready
> **Stories**: Not yet created -- run `$cgs-create-stories [epic-slug]`
...
- All stories are implemented, reviewed, and closed via `$cgs-story-done`
...
Run `$cgs-create-stories [epic-slug]` to break this epic into implementable stories.
```
...
- **Foundation + Core complete**: These are required for the Pre-Production ->
  Production gate. Run `$cgs-gate-check production` to check readiness.
- **Reminder**: Epics define scope. Stories define implementation steps. Run
  `$cgs-create-stories [epic-slug]` for each epic before developers can pick up work.
...
- **Verdict: COMPLETE** -- [N] epic(s) written. Run `$cgs-create-stories [epic-slug]` per epic.
