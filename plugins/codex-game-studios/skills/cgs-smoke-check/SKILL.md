...
description: "Codex Game Studios skill adapted from original /smoke-check. Use when the user asks for /smoke-check, $cgs-smoke-check, or this workflow. Run the critical path smoke test gate before QA hand-off. Executes the automated test suite, verifies core functionality, and produces a PASS/FAIL report. Run after a sprint's stories are implemented and before manual QA begins. A failed smoke check means the build is not ready for QA."
---

# CGS: smoke-check

> Codex adaptation: this skill is migrated from the upstream `/smoke-check` workflow. Invoke it as `$cgs-smoke-check`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
Arguments can be combined: `$cgs-smoke-check sprint --platform console`
...
1. **Test framework check**: verify `tests/` directory exists.
   If it does not: "No test directory found at `tests/`. Run `$cgs-test-setup`
...
3. **Engine detection**: read `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and
...
5. **QA plan check**: glob `production/qa/qa-plan-*.md` and take the most
   ...
   `$cgs-qa-plan sprint` before smoke-checking for best results."
...
"Engine not configured in `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`. Run
`$cgs-setup-engine` to specify the engine, then re-run `$cgs-smoke-check`."
...
3. If the `quick` argument was passed, skip this phase entirely and note:
   "Coverage scan skipped -- run `$cgs-smoke-check sprint` for full coverage
   analysis."
...
appear prominently in the report and must be resolved before `$cgs-story-done` can
...
Use `ask the user directly or use available Codex UI question tools` to batch-verify. Keep to at most 3 calls.
...
**Engine**: [engine]
**QA Plan**: [path, or "Not found -- run $cgs-qa-plan first"]
...
Fix the failures and run `$cgs-smoke-check` again to re-gate before QA hand-off."

**If verdict is PASS WITH WARNINGS:**

"Smoke check passed with warnings. The build is ready for manual QA.

Advisory items to resolve before running `$cgs-story-done` on affected stories:
...
- **PASS WITH WARNINGS does not block QA hand-off** -- it records advisory
  gaps for `$cgs-story-done` to follow up on.
- **`quick` argument** skips Phase 3 (coverage scan) and Phase 4 Batch 3.
  Use it for rapid re-checks after fixing a specific failure.
- Use `ask the user directly or use available Codex UI question tools` for all manual smoke check verification.
