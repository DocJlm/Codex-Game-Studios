---
name: cgs-soak-test
description: "Codex Game Studios skill adapted from original /soak-test. Use when the user asks for /soak-test, $cgs-soak-test, or this workflow. Generate a soak test protocol for extended play sessions. Defines what to observe, measure, and log during long play sessions to surface slow leaks, fatigue effects, and edge cases that only appear after sustained play. Primarily used in Polish and Release phases."
---

# CGS: soak-test

> Codex adaptation: this skill is migrated from the upstream `/soak-test` workflow. Invoke it as `$cgs-soak-test`. Use Codex tools and the current workspace rules; do not depend on Claude-only frontmatter, settings hooks, or slash-command runtime behavior.
...
**When to run:**
- Polish phase -- before `$cgs-gate-check release`
...
Read:
- `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` -- engine (for engine-specific memory
...
5. Run `$cgs-bug-triage sprint` after the session to integrate any S1/S2 issues

If the verdict is FAIL, run `$cgs-smoke-check` again after fixing the issues."
