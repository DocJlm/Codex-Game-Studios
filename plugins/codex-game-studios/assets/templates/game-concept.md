...
- [ ] Fill in AGENTS.md technology stack based on engine choice (`$cgs-setup-engine`)
- [ ] Create game pillars document (`$cgs-design-review` to validate)
- [ ] **Prototype core idea** (`$cgs-prototype [core-mechanic]`) — before writing GDDs, validate the concept is worth designing
- [ ] If prototype PROCEEDS: Decompose concept into systems (`$cgs-map-systems`)
- [ ] Design each system (`$cgs-design-system [system-name]`) — use prototype learnings in Tuning Knobs and Formulas sections
- [ ] Build vertical slice in Pre-Production (`$cgs-vertical-slice`) — validate full game loop before committing to Production
- [ ] Validate core loop with playtest (`$cgs-playtest-report`)
- [ ] Plan first milestone (`$cgs-sprint-plan new`)
