...
1. **Ask clarifying questions:**
   ...
   - *Use `ask the user directly or use available Codex UI question tools` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `ask the user directly or use available Codex UI question tools` to capture the decision*
...
You: [uses Write tool]
     "Created design/gdd/crafting-system.md. Would you like me to run $cgs-design-review to validate it?"
```
...
Use the `ask the user directly or use available Codex UI question tools` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `ask the user directly or use available Codex UI question tools` call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via ask the user directly or use available Codex UI question tools
