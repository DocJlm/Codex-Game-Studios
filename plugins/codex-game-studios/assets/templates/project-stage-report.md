...
- `$cgs-reverse-document [type] [path]` — [For which gap]
- `$cgs-architecture-decision` — [For which gap]
- `$cgs-sprint-plan` — [If production planning missing]
- `$cgs-milestone-review` — [If approaching deadline]
- `$cgs-onboard [role]` — [If new contributor joining]
...
*Generated by `$cgs-project-stage-detect` skill*
