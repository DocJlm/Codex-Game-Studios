...
- [ ] Design MVP-tier systems first (use `$cgs-design-system [system-name]`)
- [ ] Run `$cgs-design-review` on each completed GDD
- [ ] Run `$cgs-gate-check pre-production` when MVP systems are designed
- [ ] Validate the highest-risk systems with `$cgs-vertical-slice` before committing to Production
