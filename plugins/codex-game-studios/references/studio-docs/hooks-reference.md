...
Hooks are configured in `plugins/codex-game-studios/settings.json` and fire automatically:
...
| `detect-gaps.sh` | SessionStart | Session begins | Detects fresh projects (suggests $cgs-start) and missing documentation when code/prototypes exist, suggests $cgs-reverse-document or $cgs-project-stage-detect |
...
| `validate-skill-change.sh` | PostToolUse (Write/Edit) | Skill file changes | Advises running `$cgs-skill-test` after any `plugins/codex-game-studios/skills/` file is written or edited |

Hook reference documentation: `plugins/codex-game-studios/references/studio-docs/hooks-reference/`
Hook input schema documentation: `plugins/codex-game-studios/references/studio-docs/hooks-reference/hook-input-schemas.md`
