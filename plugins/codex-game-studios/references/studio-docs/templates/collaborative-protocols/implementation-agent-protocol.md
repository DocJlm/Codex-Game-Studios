...
2. **Ask architecture questions:**
   ...
   - *Use `ask the user directly or use available Codex UI question tools` to batch constrained architecture questions*
...
6. **Complete the story with `$cgs-story-done`:**
   - When implementation (and tests, if written) is complete, invoke `$cgs-story-done [story-file-path]`
   - This verifies acceptance criteria, checks for GDD/ADR deviations, prompts code review, and updates the story status
   - Do NOT mark a story complete manually — always go through `$cgs-story-done`
   - If no story file exists for this work (ad-hoc task), offer `$cgs-code-review` directly instead
...
File is ready. Would you like me to:
 ...
 C) Run $cgs-code-review for architecture validation"
...
You: [creates tests/combat/test_damage_calculator.gd]
     "Created test file with 6 test cases covering the acceptance criteria from the design doc.
      ...
      Running $cgs-story-done to verify acceptance criteria and close out the story."

[$cgs-story-done runs — verifies criteria, checks deviations, prompts code review, updates story status]
```
...
- Story completion is explicit — use `$cgs-story-done` to close every story, never assume done because code is written

#### Structured Decision UI

Use the `ask the user directly or use available Codex UI question tools` tool for architecture decisions and next-step choices.
Follow the **Explain → Capture** pattern:

1. **Explain first** — Describe the architectural options and trade-offs in
   conversation text.
2. **Capture the decision** — Call `ask the user directly or use available Codex UI question tools` with concise option labels.
...
**Example — architecture questions (batch):**
  ...
  ask the user directly or use available Codex UI question tools with questions:
