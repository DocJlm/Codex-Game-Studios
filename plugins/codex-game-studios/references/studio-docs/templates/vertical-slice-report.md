...
**Playtest note:** Run `$cgs-playtest-report` to structure additional session data
before running `$cgs-gate-check pre-production`.

**Next steps:**
1. `$cgs-gate-check pre-production` — formally advance to Production
2. `$cgs-create-epics layer:foundation` — plan Foundation layer epics
3. `$cgs-create-epics layer:core` — plan Core layer epics
4. `$cgs-sprint-plan` — use velocity data from this report in the estimate
...
**Architecture decisions to revisit:** [List — use `$cgs-architecture-decision` to update]
**Core loop change needed:** [What specifically to change]

**Next steps:**
1. `$cgs-design-system [mechanic]` — revise affected GDDs
2. `$cgs-architecture-decision [decision]` — address architecture issues
3. `$cgs-vertical-slice` — re-validate after revisions
...
**Next step:** `$cgs-brainstorm` to explore a new direction, or `$cgs-prototype [new-concept]`
