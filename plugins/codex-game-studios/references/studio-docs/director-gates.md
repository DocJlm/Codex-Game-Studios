...
Set once during `$cgs-start`. Edit the file directly to change it at any time.
...
Examples:
```
$cgs-brainstorm space horror           → uses global mode
$cgs-brainstorm space horror --review full   → forces full mode this run
$cgs-architecture-decision --review solo     → skips all gates this run
```
...
| `lean` | PHASE-GATEs only (`$cgs-gate-check`) — per-skill gates skipped | **Default** — solo devs and small teams; directors review at milestones only |
...
- Gate: [GATE-ID] (see plugins/codex-game-studios/references/studio-docs/director-gates.md)
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `ask the user directly or use available Codex UI question tools` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
**Trigger**: After the systems index is written by `$cgs-map-systems` — validates the
...
**Trigger**: After playtest reports are generated (`$cgs-playtest-report`), or after
...
**Trigger**: Always at `$cgs-gate-check` — spawn in parallel with TD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: After `$cgs-map-systems` Phase 3 dependency mapping is agreed but before
...
**Trigger**: After the master architecture document is drafted (`$cgs-create-architecture`
...
**Trigger**: After an individual ADR is authored (`$cgs-architecture-decision`), before
...
**Trigger**: Always at `$cgs-gate-check` — spawn in parallel with CD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: Before finalising a sprint plan (`$cgs-sprint-plan`), and after any
...
**Trigger**: At milestone review (`$cgs-milestone-review`), at mid-sprint retrospectives,
...
**Trigger**: After epics are defined by `$cgs-create-epics`, before stories are
broken out — validates the epic structure is producible before `$cgs-create-stories`
is invoked
...
**Trigger**: Always at `$cgs-gate-check` — spawn in parallel with CD-PHASE-GATE and TD-PHASE-GATE
...
**Trigger**: After the art bible is drafted (`$cgs-art-bible`), before asset production begins

**Context to pass**:
- Art bible path (`design/art/art-bible.md`)
- Game pillars and core fantasy
- Platform and performance constraints (from `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` if configured)
...
**Trigger**: Always at `$cgs-gate-check` — spawn in parallel with CD-PHASE-GATE, TD-PHASE-GATE, and PR-PHASE-GATE
...
**Trigger**: After the master architecture document is written (`$cgs-create-architecture`
...
**Trigger**: After a dev story is implemented (`$cgs-dev-story`, `$cgs-story-done`), or
as part of `$cgs-code-review`
...
**Trigger**: Before a story is accepted into a sprint — invoked by `$cgs-create-stories`,
`$cgs-story-readiness`, and `$cgs-sprint-plan` during story selection
...
done, or at `$cgs-gate-check` Production → Polish
...
at `$cgs-gate-check`), spawn all agents simultaneously:
