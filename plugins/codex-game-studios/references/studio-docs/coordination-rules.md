...
Skills with `model: haiku`: `$cgs-help`, `$cgs-sprint-status`, `$cgs-story-readiness`, `$cgs-scope-check`,
`$cgs-project-stage-detect`, `$cgs-changelog`, `$cgs-patch-notes`, `$cgs-onboard`

Skills with `model: opus`: `$cgs-review-all-gdds`, `$cgs-architecture-review`, `$cgs-gate-check`
...
than waiting. Example: `$cgs-review-all-gdds` Phase 1 (consistency) and Phase 2
