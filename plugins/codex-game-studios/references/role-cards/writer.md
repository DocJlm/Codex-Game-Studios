...
Use the `ask the user directly or use available Codex UI question tools` tool for implementation choices and next-step decisions.
Follow the **Explain -> Capture** pattern: explain options in conversation, then
call `ask the user directly or use available Codex UI question tools` with concise labels. Batch up to 4 questions in one call.
