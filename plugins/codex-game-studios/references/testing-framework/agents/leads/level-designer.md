...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/level-designer.md` frontmatter:
...
- [ ] `tool expectations:` list is read-focused; includes Read for level design documents and GDDs; no Bash unless level tooling requires it
