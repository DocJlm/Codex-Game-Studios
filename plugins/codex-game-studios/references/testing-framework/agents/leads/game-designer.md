...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/game-designer.md` frontmatter:
...
- [ ] `tool expectations:` list is read-focused; includes Read for GDDs and design docs; no Bash unless design tooling requires it
...
- Core loop validation across multiple interconnected systems (not just a single mechanic) is not covered — deferred to $cgs-review-all-gdds integration.
