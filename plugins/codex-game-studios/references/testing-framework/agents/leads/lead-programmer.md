...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/lead-programmer.md` frontmatter:
...
- [ ] `tool expectations:` list includes Read for source files; Bash may be included for static analysis or test runs; no write access outside `src/` without explicit delegation
...
- Tech debt assessment and prioritization are not covered here — deferred to $cgs-tech-debt skill integration.
