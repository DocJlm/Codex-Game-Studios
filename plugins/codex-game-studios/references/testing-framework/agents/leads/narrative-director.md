...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/narrative-director.md` frontmatter:

- [ ] `description:` field is present and domain-specific (references story, character, world-building, consistency — not generic)
- [ ] `tool expectations:` list is read-focused; includes Read for lore documents, GDDs, and narrative docs; no Bash unless justified
...
- Multi-document consistency check across a full chapter set is not covered — deferred to $cgs-review-all-gdds integration.
