...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/qa-lead.md` frontmatter:
...
- [ ] `tool expectations:` list is read-focused; may include Read for story files, test files, and coding-standards; Bash only if running test commands is required
...
- Bug severity triage (P0/P1/P2 classification) is not covered here — deferred to $cgs-bug-triage skill integration.
- Release quality gate behavior (PASS / FAIL vocabulary variant) is not covered.
- Interaction between QL-STORY-READY and story Done criteria ($cgs-story-done skill) is not covered.
