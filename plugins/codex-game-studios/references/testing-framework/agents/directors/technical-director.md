...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/technical-director.md` frontmatter:

- [ ] `description:` field is present and domain-specific (references architecture, feasibility, ADR — not generic)
- [ ] `tool expectations:` list may include Read for architecture documents; Bash only if required for technical checks
...
## Coverage Notes
- TD-ADR (Architecture Decision Record approval) is not covered — a dedicated case should be added when the $cgs-architecture-decision skill produces ADR documents.
