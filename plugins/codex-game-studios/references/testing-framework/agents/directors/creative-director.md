...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/creative-director.md` frontmatter:

- [ ] `description:` field is present and domain-specific (references creative vision, pillars, GDD alignment — not generic)
- [ ] `tool expectations:` list is read-heavy; should not include Bash unless justified by a creative workflow need
