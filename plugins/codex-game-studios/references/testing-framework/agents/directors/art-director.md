...
Verified by reading the agent's `plugins/codex-game-studios/references/role-cards/art-director.md` frontmatter:

- [ ] `description:` field is present and domain-specific (references visual identity, art bible, asset standards — not generic)
- [ ] `tool expectations:` list is read-focused; image review capability if supported; no Bash unless asset pipeline checks are justified
...
## Coverage Notes
- AD-PHASE-GATE (full visual phase advancement) is not covered — deferred to integration with $cgs-gate-check skill.
