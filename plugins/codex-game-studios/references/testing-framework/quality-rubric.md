...
Used by `$cgs-skill-test category [name|all]` to evaluate skills beyond structural compliance.
...
> **Exceptions:**
> - `design-review`: Has `Write, Edit` in tool expectations to support an optional "Revise now" path (all writes gated behind user approval) and to write review logs. R1 is satisfied because the reviewed document is never silently modified.
...
| Metric | PASS criteria |
|---|---|
| **U1 — Passes all 7 static checks** | `$cgs-skill-test static [name]` returns COMPLIANT with 0 FAILs |
...
| **O3 — Toolset matches role** | `tool expectations` in frontmatter matches the operational (not coding) nature of the role |
