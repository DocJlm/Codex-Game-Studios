...
├── quality-rubric.md      ← category-specific pass/fail metrics for $cgs-skill-test category
│
...
│
└── results/               ← test run outputs (written by $cgs-skill-test spec, gitignored)
```
...
```
$cgs-skill-test static [skill-name]     # Check one skill (7 checks)
$cgs-skill-test static all              # Check all 73 skills
```

### Run a behavioral spec test

```
$cgs-skill-test spec gate-check         # Evaluate a skill against its written spec
$cgs-skill-test spec design-review
```

### Check against category rubric

```
$cgs-skill-test category gate-check     # Evaluate one skill against its category metrics
$cgs-skill-test category all            # Run rubric checks across all categorized skills
```

### See full coverage picture

```
$cgs-skill-test audit                   # Skills + agents: has-spec, last tested, result
```

### Improve a failing skill

```
$cgs-skill-improve gate-check           # Test → diagnose → propose fix → retest loop
```
...
- `$cgs-skill-test spec [name]` will offer to update `last_spec` and `last_spec_result`
- `$cgs-skill-test category [name]` will offer to update `last_category` and `last_category_result`
- `last_static` and `last_static_result` are updated manually or via `$cgs-skill-improve`
...
4. Run `$cgs-skill-test spec [skill-name]` to validate it
...
report that `catalog.yaml` is missing and suggest running `$cgs-skill-test audit` to
initialize it.
