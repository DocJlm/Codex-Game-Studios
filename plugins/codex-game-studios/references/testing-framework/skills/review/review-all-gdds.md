# Skill Test Spec: $cgs-review-all-gdds

## Skill Summary

`$cgs-review-all-gdds` is an Opus-tier skill that performs a holistic cross-GDD review
...
- [ ] Next-step handoff to `$cgs-architecture-review` or `$cgs-create-architecture` is present
...
- [ ] Skill suggests running `$cgs-design-system system-B` to resolve the gap
...
3. Skill recommends running `$cgs-brainstorm` and `$cgs-design-system` before re-running
...
- [ ] Skill recommends the correct next action (`$cgs-brainstorm` or `$cgs-design-system`)
...
- [ ] Ends with appropriate handoff: MAJOR ISSUES → fix and re-run; MINOR ISSUES → may proceed with awareness; CONSISTENT → `$cgs-create-architecture`
