# Skill Test Spec: $cgs-design-review

## Skill Summary

`$cgs-design-review` reads a game design document (GDD) and evaluates it against
...
- [ ] Does NOT require "May I write" language (read-only skill — `tool expectations` excludes Write/Edit)
...
**Input:** `$cgs-design-review design/gdd/light-manipulation.md`
...
**Input:** `$cgs-design-review design/gdd/[document].md`
...
**Input:** `$cgs-design-review design/gdd/nonexistent.md`
...
**Input:** `$cgs-design-review design/gdd/light-manipulation.md` (with full review mode active)
...
- [ ] Ends with recommended next step (e.g., fix issues and re-run, or proceed to `$cgs-map-systems`)
...
- Cross-system consistency checking (Case 3 in the skill's own phase list) is
  ...
  this is covered by the `$cgs-review-all-gdds` spec instead.
