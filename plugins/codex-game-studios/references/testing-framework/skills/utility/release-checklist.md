# Skill Test Spec: $cgs-release-checklist

## Skill Summary

`$cgs-release-checklist` generates an internal release readiness checklist covering:
...
(that is `$cgs-launch-checklist`). When a previous release checklist exists, it shows
...
after a "May I write" ask. No director gates apply — `$cgs-gate-check` handles
...
- [ ] Has a next-step handoff (e.g., `$cgs-launch-checklist` for external or `$cgs-gate-check` for phase)

---

## Director Gate Checks

None. `$cgs-release-checklist` is an internal audit utility. Formal phase advancement
is managed by `$cgs-gate-check`.
