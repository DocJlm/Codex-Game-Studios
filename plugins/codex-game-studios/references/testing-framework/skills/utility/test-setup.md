# Skill Test Spec: $cgs-test-setup

## Skill Summary

`$cgs-test-setup` scaffolds the test framework for the project based on the
...
- [ ] Has a next-step handoff (e.g., `$cgs-test-helpers` to generate helper utilities)

---

## Director Gate Checks

None. `$cgs-test-setup` is a scaffolding utility. No director gates apply.
...
### Case 4: No Engine Configured — Redirects to $cgs-setup-engine
...
3. Skill suggests running `$cgs-setup-engine` first
4. No directories or files are created

**Assertions:**
- [ ] Error message explicitly states engine is not configured
- [ ] `$cgs-setup-engine` is suggested as the next step
