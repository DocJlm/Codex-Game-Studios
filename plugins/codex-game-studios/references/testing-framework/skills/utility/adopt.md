# Skill Test Spec: $cgs-adopt

## Skill Summary

`$cgs-adopt` audits an existing project's artifacts — GDDs, ADRs, stories, infrastructure
...
after explicit user approval via `ask the user directly or use available Codex UI question tools`.

This skill is distinct from `$cgs-project-stage-detect` (which checks what exists).
`$cgs-adopt` checks whether what exists will actually work with the template's skills.
...
Verified automatically by `$cgs-skill-test static` — no fixture needed.
...
- [ ] Contains "May I write" or `ask the user directly or use available Codex UI question tools` language before writing the adoption plan
...
None. `$cgs-adopt` is a brownfield audit utility. No director gates apply.
...
8. Uses `ask the user directly or use available Codex UI question tools` to ask about writing the plan; user selects write
...
- [ ] `ask the user directly or use available Codex UI question tools` is used before writing the adoption plan
...
4. Phase 4 builds ordered migration plan:
   - Step 1 (BLOCKING): Add `## Status` to `adr-0001.md` — command: `$cgs-architecture-decision retrofit`
   - Step 2 (HIGH): Run `$cgs-architecture-review` to bootstrap tr-registry.yaml
   - Step 3 (HIGH): Add Acceptance Criteria to `combat.md` — command: `$cgs-design-system retrofit`
...
6. `ask the user directly or use available Codex UI question tools` asks to write the plan; writes after approval
...
- [ ] `ask the user directly or use available Codex UI question tools` is used before writing
...
6. `ask the user directly or use available Codex UI question tools` to write plan; writes after approval
...
### Case 4: No Artifacts Found — Fresh project, guidance to run $cgs-start
...
3. Uses `ask the user directly or use available Codex UI question tools`:
   ...
   - Options: "Run `$cgs-start`", "My artifacts are in a non-standard location", "Cancel"
4. Skill stops — does not proceed to audit regardless of user selection

**Assertions:**
- [ ] `ask the user directly or use available Codex UI question tools` is used (not a plain text message) when no artifacts are found
- [ ] `$cgs-start` is presented as a named option
...
4. No `$cgs-gate-check` is invoked during the skill run
...
- [ ] Uses `ask the user directly or use available Codex UI question tools` before writing the adoption plan file
