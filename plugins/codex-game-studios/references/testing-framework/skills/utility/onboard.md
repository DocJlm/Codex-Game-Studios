# Skill Test Spec: $cgs-onboard

## Skill Summary

`$cgs-onboard` generates a contextual project onboarding summary tailored for a new
...
The skill optionally accepts a role argument (e.g., `$cgs-onboard artist`) to tailor
...
None. `$cgs-onboard` is a read-only orientation skill. No director gates apply.
...
4. Next-step suggestions are appropriate for Production stage (e.g., `$cgs-sprint-status`,
   `$cgs-dev-story`)
...
### Case 2: Fresh Project — No engine, no sprint, suggests $cgs-start
...
3. Output explains the onboarding workflow: `$cgs-start` → `$cgs-setup-engine` → `$cgs-brainstorm`
4. Skill suggests running `$cgs-start` as the immediate next step
5. Verdict is ONBOARDING COMPLETE (informational, not a failure)

**Assertions:**
- [ ] Output explicitly mentions the project is not yet configured
- [ ] `$cgs-start` is recommended as the next step
...
3. Skill provides remediation: "Run `$cgs-start` to initialize the project configuration"
4. No partial summary is generated

**Assertions:**
- [ ] Error message clearly identifies the missing file as AGENTS.md
- [ ] Remediation step (`$cgs-start`) is explicitly named
