# Skill Test Spec: $cgs-setup-engine

## Skill Summary

`$cgs-setup-engine` configures the project's engine, language, rendering backend,
...
(e.g., `$cgs-setup-engine godot`) to skip the engine-selection step. For each
...
- [ ] Has a next-step handoff (e.g., `$cgs-brainstorm` or `$cgs-start` depending on flow)

---

## Director Gate Checks

None. `$cgs-setup-engine` is a technical configuration skill. No director gates apply.
