# Skill Test Spec: $cgs-bug-report

## Skill Summary

`$cgs-bug-report` creates a structured bug report document from a user description.
...
- [ ] Has a next-step handoff (e.g., `$cgs-bug-triage` to reprioritize, `$cgs-hotfix` for critical)

---

## Director Gate Checks

None. `$cgs-bug-report` is an operational documentation skill. No director gates apply.
...
**Input:** `$cgs-bug-report` (user describes: "Game crashes when player enters the boss arena")
...
**Input:** `$cgs-bug-report` (user describes: "Audio randomly stops working")
...
**Input:** `$cgs-bug-report` (user describes: "After finishing a level, the save system
