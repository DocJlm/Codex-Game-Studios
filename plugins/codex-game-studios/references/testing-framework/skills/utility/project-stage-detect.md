# Skill Test Spec: $cgs-project-stage-detect

## Skill Summary

`$cgs-project-stage-detect` automatically analyzes project artifacts to determine
...
when `$cgs-gate-check` passes and the user confirms advancement. The skill reports
...
- [ ] Has a next-step handoff (e.g., `$cgs-gate-check` to formally advance stage)

---

## Director Gate Checks

None. `$cgs-project-stage-detect` is a read-only detection utility. No director
gates apply.
...
**Input:** `$cgs-project-stage-detect`
...
5. Next step: continue with `$cgs-sprint-plan` or `$cgs-dev-story`
...
5. Skill recommends running `$cgs-gate-check` to formalize and write stage.txt

**Assertions:**
- [ ] Inferred stage is Production
- [ ] Confidence is MEDIUM (not HIGH, since stage.txt is absent)
- [ ] Recommendation to run `$cgs-gate-check` is present
...
5. Skill suggests `$cgs-start` to begin the onboarding workflow

**Assertions:**
- [ ] Inferred stage is Concept
- [ ] Output lists the artifacts that were checked (and found absent)
- [ ] `$cgs-start` is suggested as the next step
- [ ] No files are written
...
5. Skill suggests reviewing stage.txt manually or running `$cgs-gate-check`
...
- [ ] Does not write stage.txt (that is `$cgs-gate-check`'s responsibility)
