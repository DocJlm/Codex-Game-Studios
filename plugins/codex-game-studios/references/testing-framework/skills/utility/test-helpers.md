# Skill Test Spec: $cgs-test-helpers

## Skill Summary

`$cgs-test-helpers` generates engine-specific test helper utilities for the project's
...
None. `$cgs-test-helpers` is a scaffolding utility. No director gates apply.
...
**Input:** `$cgs-test-helpers player-factory`
...
### Case 2: No Test Setup Exists — Redirects to $cgs-test-setup
...
3. Skill suggests running `$cgs-test-setup` before generating helpers
4. No helper file is created

**Assertions:**
- [ ] Error message identifies the missing tests/ directory
- [ ] `$cgs-test-setup` is suggested as the prerequisite step
...
**Input:** `$cgs-test-helpers enemy-factory`
...
**Input:** `$cgs-test-helpers inventory-factory`
