# Skill Test Spec: $cgs-day-one-patch

## Skill Summary

`$cgs-day-one-patch` prepares a day-one patch plan for issues that are known at
...
triggers guidance to run `$cgs-hotfix` before the patch. No director gates apply.
...
- [ ] Has a next-step handoff (e.g., `$cgs-hotfix` for P0 issues, `$cgs-release-checklist` for follow-up)

---

## Director Gate Checks

None. `$cgs-day-one-patch` is a release planning utility. No director gates apply.
...
### Case 2: Critical Issue Discovered Post-Ship — P0, Triggers $cgs-hotfix Guidance
...
4. Skill explicitly directs: "Run `$cgs-hotfix` to resolve this issue first"
...
- [ ] `$cgs-hotfix` is explicitly directed for the P0 issue
...
- [ ] Escalates CRITICAL (P0) bugs with explicit `$cgs-hotfix` guidance
...
- The patch notes player communication document (`$cgs-patch-notes`) is a separate
