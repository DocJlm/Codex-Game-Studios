# Skill Test Spec: $cgs-playtest-report

## Skill Summary

`$cgs-playtest-report` generates a structured playtest report from session notes or
...
- [ ] Has a next-step handoff (e.g., `$cgs-bug-report` for new issues found, `$cgs-design-review` for feedback)

---

## Director Gate Checks

None. `$cgs-playtest-report` is a documentation utility. The CD-PLAYTEST gate is a
...
**Input:** `$cgs-playtest-report` (user pastes session notes)
...
5. Next Steps: suggests `$cgs-bug-report` for the framerate issue and `$cgs-design-review`
...
**Input:** `$cgs-playtest-report` (3-tester session)
...
- [ ] `$cgs-bug-report` is NOT suggested for the already-reported issue
