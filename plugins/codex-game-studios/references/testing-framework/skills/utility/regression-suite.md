# Skill Test Spec: $cgs-regression-suite

## Skill Summary

`$cgs-regression-suite` maps test coverage to GDD requirements: it reads the
...
- [ ] Has a next-step handoff (e.g., `$cgs-test-setup` if framework missing, `$cgs-qa-plan` if plan missing)

---

## Director Gate Checks

None. `$cgs-regression-suite` is a QA analysis utility. No director gates apply.
...
**Input:** `$cgs-regression-suite sprint-004`
