# Skill Test Spec: $cgs-prototype

## Skill Summary

`$cgs-prototype` manages a rapid prototyping workflow for validating a game mechanic
...
- [ ] Has a next-step handoff (e.g., `$cgs-design-system` to formalize, or archive)
...
**Input:** `$cgs-prototype grapple-hook`
...
**Input:** `$cgs-prototype grapple-hook` (prototype session complete)
...
2. Recommendation in findings.md: "Mechanic validated — recommend proceeding
   to `$cgs-design-system` for full specification"
3. Skill handoff message explicitly suggests `$cgs-design-system grapple-hook`
4. Verdict is PROTOTYPE COMPLETE

**Assertions:**
- [ ] `findings.md` contains an explicit recommendation
- [ ] Recommendation references `$cgs-design-system` when mechanic is validated
...
**Input:** `$cgs-prototype procedural-dialogue`
