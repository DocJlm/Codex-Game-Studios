# Skill Test Spec: $cgs-smoke-check

## Skill Summary

`$cgs-smoke-check` is the gate between implementation and QA hand-off. It detects the
test environment, runs the automated test suite (via Bash), scans test coverage
against sprint stories, and uses `ask the user directly or use available Codex UI question tools` to batch-verify manual smoke
...
- [ ] Has a next-step handoff (e.g., `$cgs-bug-report` on FAIL, QA hand-off guidance on PASS)

---

## Director Gate Checks

None. `$cgs-smoke-check` is a pre-QA utility skill. No director gates apply.
...
5. Uses `ask the user directly or use available Codex UI question tools` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `ask the user directly or use available Codex UI question tools` is used for manual smoke check batches
...
7. Delivers FAIL verdict with message: "The smoke check failed. Do not hand off to
   ...
   then re-running `$cgs-smoke-check`

**Assertions:**
- [ ] Failing test names are listed in the report
- [ ] Verdict is FAIL
- [ ] Post-verdict message directs developer to fix failures before QA hand-off
- [ ] `$cgs-smoke-check` re-run is suggested after fixing

---

### Case 3: Manual Confirmation — ask the user directly or use available Codex UI question tools used, PASS WITH WARNINGS
...
3. `ask the user directly or use available Codex UI question tools` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
5. Verdict is PASS WITH WARNINGS — build ready for QA, but MISSING entry must be
   resolved before `$cgs-story-done` closes the affected story
6. Asks to write report; writes after approval

**Assertions:**
- [ ] `ask the user directly or use available Codex UI question tools` is used for manual smoke check batches (not inline text prompts)
- [ ] MISSING test coverage entry appears in the report
- [ ] Verdict is PASS WITH WARNINGS (not PASS, not FAIL)
- [ ] Advisory note explains MISSING entry must be resolved before `$cgs-story-done`
...
2. Skill outputs: "No test directory found at `tests/`. Run `$cgs-test-setup` to
...
- [ ] `$cgs-test-setup` is suggested as the remediation step
...
4. No `$cgs-gate-check` is invoked
...
- [ ] Uses `ask the user directly or use available Codex UI question tools` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific ask the user directly or use available Codex UI question tools batches and a
