# Skill Test Spec: $cgs-bug-triage

## Skill Summary

`$cgs-bug-triage` reads all open bug reports in `production/bugs/` and produces a
...
- [ ] Has a next-step handoff (e.g., `$cgs-bug-report` to create new reports, `$cgs-hotfix` for critical bugs)

---

## Director Gate Checks

None. `$cgs-bug-triage` is a read-only advisory skill. No director gates apply.
...
### Case 2: No Bug Reports Found — Guidance to run $cgs-bug-report
...
3. Skill suggests running `$cgs-bug-report` to create a bug report
4. No triage table is produced

**Assertions:**
- [ ] Output explicitly states no bugs were found
- [ ] `$cgs-bug-report` is suggested as the next step
