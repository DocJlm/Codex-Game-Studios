# Skill Test Spec: $cgs-hotfix

## Skill Summary

`$cgs-hotfix` manages an emergency fix workflow: it creates a hotfix branch from
main, applies a targeted fix to the identified file(s), runs `$cgs-smoke-check` to
...
- [ ] Has a next-step handoff (e.g., `$cgs-bug-report` to document the issue, or version bump)
...
**Input:** `$cgs-hotfix` (user describes the crash and affected file)
...
5. Skill runs `$cgs-smoke-check` — PASS
...
- [ ] `$cgs-smoke-check` runs after the fix is applied
...
- `$cgs-smoke-check` returns FAIL: "Player health clamping regression detected"

**Input:** `$cgs-hotfix`

**Expected behavior:**
1. Skill applies the fix and runs `$cgs-smoke-check`
...
**Fixture:**
- User invokes `$cgs-hotfix` with a vague description: "something is broken on level 3"
- No repro steps provided

**Input:** `$cgs-hotfix` (vague description)
...
- [ ] Runs `$cgs-smoke-check` after applying the fix
