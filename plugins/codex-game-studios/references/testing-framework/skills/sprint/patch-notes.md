# Skill Test Spec: $cgs-patch-notes

## Skill Summary

`$cgs-patch-notes` is a Haiku-tier skill that generates player-facing patch notes
...
### Case 2: No Changelog Found — Directed to run $cgs-changelog first
...
2. Skill outputs: "No changelog found — run $cgs-changelog first to generate one"
3. No patch notes are generated; no file is written

**Assertions:**
- [ ] Skill does not crash when changelog is absent
- [ ] Output explicitly directs user to run `$cgs-changelog`
...
**Fixture:**
- `plugins/codex-game-studios/assets/templates/patch-notes-template.md` exists with a structured header format
- `docs/CHANGELOG.md` exists with player-facing entries
