# Skill Test Spec: $cgs-changelog

## Skill Summary

`$cgs-changelog` is a Haiku-tier skill that auto-generates a developer-facing
...
- [ ] Has a next-step handoff (e.g., run $cgs-patch-notes for player-facing version)
...
- The `$cgs-patch-notes` skill should be run after `$cgs-changelog` for player-facing
