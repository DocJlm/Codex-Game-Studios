# Skill Test Spec: $cgs-sprint-status

## Skill Summary

`$cgs-sprint-status` is a Haiku-tier read-only skill that reads the current active
...
None. `$cgs-sprint-status` is a read-only reporting skill; no gates are invoked.
...
3. Skill suggests running `$cgs-milestone-review` or `$cgs-sprint-plan` as next steps

**Assertions:**
- [ ] Verdict is ON TRACK or SPRINT COMPLETE when all stories are Complete
- [ ] Output notes that the sprint is fully done
- [ ] Next-step suggestion references `$cgs-milestone-review` or `$cgs-sprint-plan`
- [ ] No files are written

---

### Case 3: No Active Sprint File — Guidance to run $cgs-sprint-plan
...
4. Skill suggests running `$cgs-sprint-plan` to create one

**Assertions:**
- [ ] Skill does not error or crash when no sprint file exists
- [ ] Output clearly states no active sprint was found
- [ ] Output recommends `$cgs-sprint-plan` as the next action
