# Skill Test Spec: $cgs-ux-design

## Skill Summary

`$cgs-ux-design` is a guided, section-by-section UX spec authoring skill. It produces
...
The skill has no inline director gates — `$cgs-ux-review` is the separate review step.
...
- [ ] Has a next-step handoff (e.g., `$cgs-ux-review` to validate the completed spec)

---

## Director Gate Checks

None. `$cgs-ux-design` has no inline director gates. `$cgs-ux-review` is the separate
...
6. Skill suggests running `$cgs-ux-review` as the next step
...
- [ ] Handoff to `$cgs-ux-review` is at the end
- [ ] Verdict is COMPLETE
...
**Input:** `$cgs-ux-design inventory-screen`
...
- [ ] Handoff suggests both `$cgs-ux-review` and writing the missing GDD
...
2. Skill outputs a usage error: "Screen name required. Usage: `$cgs-ux-design [screen-name]`"
3. Skill provides examples: `$cgs-ux-design hud`, `$cgs-ux-design main-menu`, `$cgs-ux-design inventory`
...
**Input:** `$cgs-ux-design settings-menu`
...
- [ ] Ends with handoff to `$cgs-ux-review`
...
- Interaction state enumeration (normal/hover/focus/disabled/error) is a core
  requirement of each spec; the `$cgs-ux-review` skill checks for completeness.
