# Skill Test Spec: $cgs-create-architecture

## Skill Summary

`$cgs-create-architecture` guides the user through section-by-section authoring of a
...
- [ ] Has a next-step handoff at the end (`$cgs-architecture-review` or `$cgs-create-control-manifest`)
...
- [ ] Next-step handoff to `$cgs-architecture-review` or `$cgs-create-control-manifest` is present
...
- [ ] Ends with next-step handoff: `$cgs-architecture-review` or `$cgs-create-control-manifest`
...
- The required section list for architecture documents is defined in the skill
  body and in the `$cgs-architecture-review` skill — not re-enumerated here.
