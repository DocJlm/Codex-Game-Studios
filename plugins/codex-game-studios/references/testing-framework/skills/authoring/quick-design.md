# Skill Test Spec: $cgs-quick-design

## Skill Summary

`$cgs-quick-design` produces a lightweight design spec for features too small to
...
for a quick-design, the skill redirects to `$cgs-design-system` instead.
...
- [ ] Mentions scope check: redirects to `$cgs-design-system` if scope exceeds sub-4h threshold
...
**Input:** `$cgs-quick-design [feature-name]`
...
### Case 2: Failure Path — Scope check fails; redirected to $cgs-design-system
...
**Input:** `$cgs-quick-design` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Skill outputs a usage error: "No feature name specified. Usage: $cgs-quick-design [feature-name]"
3. Skill provides an example: `$cgs-quick-design pause-menu-settings`
...
- [ ] Full GDD review path (`$cgs-design-system`) is mentioned as the alternative for larger features

---

## Protocol Compliance

- [ ] Scope check runs before drafting (redirects to `$cgs-design-system` if scope too large)
...
- [ ] Ends with next-step handoff (e.g., proceed to implementation or `$cgs-dev-story`)
