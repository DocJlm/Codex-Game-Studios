# Skill Test Spec: $cgs-test-evidence-review

## Skill Summary

`$cgs-test-evidence-review` performs a quality review of test files in `tests/`,
...
**Input:** `$cgs-test-evidence-review tests/unit/combat/`
...
**Input:** `$cgs-test-evidence-review tests/unit/ui/hud_update_test.gd`
...
**Input:** `$cgs-test-evidence-review tests/unit/networking/auth_test.gd`
...
**Fixture:**
- User calls `$cgs-test-evidence-review tests/unit/audio/`
- `tests/unit/audio/` directory does not exist

**Input:** `$cgs-test-evidence-review tests/unit/audio/`

**Expected behavior:**
1. Skill attempts to read files in `tests/unit/audio/` — not found
2. Skill outputs: "No test files found at `tests/unit/audio/` — run `$cgs-test-setup` to scaffold test directories"
3. No verdict is emitted

**Assertions:**
- [ ] Skill does not crash when path does not exist
- [ ] Output names the attempted path in the message
- [ ] Output recommends `$cgs-test-setup` for scaffolding
...
4. Output notes: "For full test coverage gate, run `$cgs-gate-check` which invokes QL-TEST-COVERAGE"
