# Skill Test Spec: $cgs-code-review

## Skill Summary

`$cgs-code-review` performs an architectural code review of source files in `src/`,
...
**Input:** `$cgs-code-review src/gameplay/health_component.gd`
...
**Input:** `$cgs-code-review src/ui/inventory_ui.gd`
...
**Input:** `$cgs-code-review src/core/save_system.gd`
...
**Fixture:**
- User calls `$cgs-code-review src/networking/`
- `src/networking/` directory does not exist

**Input:** `$cgs-code-review src/networking/`
...
**Input:** `$cgs-code-review src/gameplay/loot_system.gd`
...
- Test coverage checks (verifying corresponding test files exist) are a stretch
  goal not tested here; that is primarily the domain of `$cgs-test-evidence-review`.
