# Skill Test Spec: $cgs-consistency-check

## Skill Summary

`$cgs-consistency-check` scans all GDDs in `design/gdd/` and checks for internal
...
- [ ] Skill suggests running `$cgs-design-system system-B` to create the missing GDD
...
3. Skill outputs an error: "No GDDs found in `design/gdd/`. Run `$cgs-design-system` to create GDDs first."
...
- [ ] Skill recommends the correct next action (`$cgs-design-system`)
...
- This skill checks for structural consistency between GDDs. Deep design theory
  analysis (pillar drift, dominant strategies) is handled by `$cgs-review-all-gdds`.
