# Skill Test Spec: $cgs-asset-audit

## Skill Summary

`$cgs-asset-audit` audits the `assets/` directory for naming convention compliance,
...
**Expected behavior:**
1. Skill reads GDD references to find expected assets (cross-references with `$cgs-content-audit` scope)
...
- The interaction between `$cgs-asset-audit` and `$cgs-content-audit` (both check GDD
  references vs. assets) is intentional overlap; `$cgs-asset-audit` focuses on
  compliance while `$cgs-content-audit` focuses on completeness.
