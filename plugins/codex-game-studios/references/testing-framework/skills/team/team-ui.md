...
Uses `ask the user directly or use available Codex UI question tools` at each phase transition. Delegates all file writes to
sub-agents and sub-skills (`$cgs-ux-design`, `ui-programmer`). Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `$cgs-ux-review`, `$cgs-code-review`,
`$cgs-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `$cgs-ux-review`, `$cgs-code-review`, `$cgs-team-polish`)
...
- Engine UI specialist configured in `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`

**Input:** `$cgs-team-ui inventory screen`
...
2. Phase 1b — `$cgs-ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `ask the user directly or use available Codex UI question tools` confirms spec before review
3. Phase 1c — `$cgs-ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `ask the user directly or use available Codex UI question tools` confirms before Phase 3
...
- [ ] Next steps include `$cgs-ux-review`, `$cgs-code-review`, `$cgs-team-polish`
...
- `$cgs-ux-review` returns verdict NEEDS REVISION with specific concerns flagged (e.g., gamepad navigation flow incomplete, contrast ratio below minimum)

**Input:** `$cgs-team-ui inventory screen`

**Expected behavior:**
1. Phase 1a + 1b complete — UX spec produced
2. Phase 1c — `$cgs-ux-review design/ux/inventory-screen.md` returns NEEDS REVISION
3. Skill does NOT advance to Phase 2
4. `ask the user directly or use available Codex UI question tools` presented with the specific flagged concerns and options:
...
5. If user chooses (a): ux-designer revises spec, `$cgs-ux-review` re-run; loop continues until APPROVED or user overrides
...
- [ ] `ask the user directly or use available Codex UI question tools` presents the specific flagged concerns before offering options
...
**Input:** `$cgs-team-ui inventory screen` (resuming from Phase 3 complete)
...
5. `ask the user directly or use available Codex UI question tools` presents all three review results before Phase 5 begins
...
- [ ] `ask the user directly or use available Codex UI question tools` shows all three review streams' results together before Phase 5 approval
...
**Input:** `$cgs-team-ui settings menu`
...
3. `ask the user directly or use available Codex UI question tools` presented with options:
...
- [ ] `ask the user directly or use available Codex UI question tools` offers a "create pattern library first" option (referencing `$cgs-ux-design patterns`)
...
- [ ] Next steps present at end: `$cgs-ux-review`, `$cgs-code-review`, `$cgs-team-polish`

---

## Coverage Notes

- The HUD-specific path (`$cgs-ux-design hud` + `hud-design.md` template + visual budget check in Phase 5)
