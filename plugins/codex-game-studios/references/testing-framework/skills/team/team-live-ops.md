# Skill Test Spec: $cgs-team-live-ops
...
- [ ] Has a next-step handoff at the end referencing `$cgs-design-review`, `$cgs-sprint-plan`, and `$cgs-team-release`
...
**Input:** `$cgs-team-live-ops "Season 2: The Frozen Wastes"`
...
2. ask the user directly or use available Codex UI question tools: user approves Phase 1 output before Phase 2 begins
...
8. ask the user directly or use available Codex UI question tools: user approves the full season plan
...
- [ ] Next steps reference `$cgs-design-review`, `$cgs-sprint-plan`, and `$cgs-team-release`
...
**Input:** `$cgs-team-live-ops "Season 3: Shadow Tournament"`
...
4. ask the user directly or use available Codex UI question tools presented with resolution options before season plan approval is offered
...
- [ ] ask the user directly or use available Codex UI question tools is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
**Input:** `$cgs-team-live-ops` (no argument)

**Expected behavior:**
1. Phase 1: No argument detected
2. Outputs: "Usage: `$cgs-team-live-ops [season name or event description]` — Provide the name or description of the season or live event to plan."
3. Skill exits immediately without spawning any subagents

**Assertions:**
- [ ] Skill does NOT guess a season name or fabricate a scope
- [ ] Error message includes the correct usage format with the usage note
...
**Input:** `$cgs-team-live-ops "Season 1: The First Thaw"` (observed at Phase 3/4 transition)
...
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via ask the user directly or use available Codex UI question tools
...
**Input:** `$cgs-team-live-ops "Season 4: Desert Heat"`
...
- [ ] `ask the user directly or use available Codex UI question tools` used at every phase transition — user approves before the next phase begins
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with ask the user directly or use available Codex UI question tools options (skip / retry / stop)
...
- [ ] Next steps always include `$cgs-design-review`, `$cgs-sprint-plan`, and `$cgs-team-release`
