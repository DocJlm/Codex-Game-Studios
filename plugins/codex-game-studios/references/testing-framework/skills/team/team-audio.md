# Skill Test Spec: $cgs-team-audio
...
`ask the user directly or use available Codex UI question tools` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] Has a next-step handoff at the end (references `$cgs-dev-story`, `$cgs-asset-audit`)
- [ ] Error Recovery Protocol section is present
- [ ] `ask the user directly or use available Codex UI question tools` is used at step transitions before proceeding
...
- Engine is configured in `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`
- No accessibility gaps exist in the planned audio event list

**Input:** `$cgs-team-audio combat`
...
3. `ask the user directly or use available Codex UI question tools` presents audio direction; user approves before Step 2 begins
...
5. `ask the user directly or use available Codex UI question tools` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
7. `ask the user directly or use available Codex UI question tools` presents technical plan; user approves before Step 4 begins
...
- [ ] `ask the user directly or use available Codex UI question tools` appears after Step 1 output and before Step 2 launch
...
- [ ] gameplay-programmer is not launched until Step 3 `ask the user directly or use available Codex UI question tools` is approved
...
**Input:** `$cgs-team-audio stealth` (Step 2 scenario)
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `ask the user directly or use available Codex UI question tools`
4. `ask the user directly or use available Codex UI question tools` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `ask the user directly or use available Codex UI question tools` surfaces the gap before Step 3 is launched
...
**Input:** `$cgs-team-audio` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `$cgs-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`)"
...
**Input:** `$cgs-team-audio main menu`
...
**Fixture:**
- Engine is NOT configured in `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` (shows `[TO BE CONFIGURED]`)
- GDD for the target feature exists
- Sound bible may or may not exist

**Input:** `$cgs-team-audio boss encounter`

**Expected behavior:**
1. Context gathering: orchestrator reads `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` and detects no engine is configured
...
- [ ] `ask the user directly or use available Codex UI question tools` is used after every step output before the next step launches
...
- [ ] Next Steps handoff references `$cgs-dev-story` and `$cgs-asset-audit`
