# Skill Test Spec: $cgs-team-combat
...
(parallel) → Integration → Validation → Sign-off. Uses `ask the user directly or use available Codex UI question tools` at each
phase transition. Delegates all file writes to sub-agents. Produces a summary report
with verdict COMPLETE / NEEDS WORK / BLOCKED and handoffs to `$cgs-code-review`,
`$cgs-balance-check`, and `$cgs-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `$cgs-code-review`, `$cgs-balance-check`, `$cgs-team-polish`)
- [ ] Error Recovery Protocol section is present with all four recovery steps
- [ ] Uses `ask the user directly or use available Codex UI question tools` at phase transitions for user approval before proceeding
- [ ] Phase 3 is explicitly marked as parallel (gameplay-programmer, ai-programmer, technical-artist, sound-designer)
- [ ] Phase 2 includes spawning the primary engine specialist (read from `plugins/codex-game-studios/references/studio-docs/technical-preferences.md`)
...
- Engine is configured in `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` (Engine Specialists section filled)
- No existing GDD for the requested combat feature

**Input:** `$cgs-team-combat parry and riposte system`
...
2. Phase 2 — gameplay-programmer + ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `ask the user directly or use available Codex UI question tools` presented with architecture options before Phase 3 begins
...
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `ask the user directly or use available Codex UI question tools` confirms integration before Phase 5
...
7. Next steps listed: `$cgs-code-review`, `$cgs-balance-check`, `$cgs-team-polish`

**Assertions:**
- [ ] `ask the user directly or use available Codex UI question tools` called at each phase gate (at minimum before Phase 3 and before Phase 5)
...
- [ ] Next steps include `$cgs-code-review`, `$cgs-balance-check`, `$cgs-team-polish`
...
4. `ask the user directly or use available Codex UI question tools` presented with options: (a) Skip ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `$cgs-architecture-decision` first
...
- [ ] `ask the user directly or use available Codex UI question tools` offers at minimum three options: skip / retry / stop
...
- [ ] Blocked reason references the ADR and suggests `$cgs-architecture-decision`
...
**Input:** `$cgs-team-combat parry and riposte system` (resuming from Phase 2 complete)
...
**Fixture:**
- `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` has Engine Specialists section populated (e.g., Primary: godot-specialist)
...
2. Skill reads `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` Engine Specialists section to identify the primary engine specialist agent type
...
6. `ask the user directly or use available Codex UI question tools` includes engine specialist's notes alongside the architecture sketch

**Assertions:**
- [ ] Engine specialist agent type is read from `plugins/codex-game-studios/references/studio-docs/technical-preferences.md` — not hardcoded
...
- [ ] `ask the user directly or use available Codex UI question tools` used at each phase transition — user approves before pipeline advances
...
- [ ] Next steps present at end of output: `$cgs-code-review`, `$cgs-balance-check`, `$cgs-team-polish`
...
- "Retry with narrower scope" error recovery option is listed in assertions but its full
  recursive behavior (splitting via `$cgs-create-stories`) is covered by the `$cgs-create-stories` spec.
