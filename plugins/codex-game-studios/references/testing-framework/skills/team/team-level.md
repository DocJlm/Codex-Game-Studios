# Skill Test Spec: $cgs-team-level
...
document saved to `design/levels/[level-name].md`. Uses `ask the user directly or use available Codex UI question tools` at each
step transition. Delegates all file writes to sub-agents. Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `$cgs-design-review`, `$cgs-dev-story`,
`$cgs-qa-plan`.
...
- [ ] Has a next-step handoff at the end (references `$cgs-design-review`, `$cgs-dev-story`, `$cgs-qa-plan`)
- [ ] Error Recovery Protocol section is present with all four recovery steps
- [ ] Uses `ask the user directly or use available Codex UI question tools` at step transitions for user approval before proceeding
...
**Input:** `$cgs-team-level forest dungeon`
...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `ask the user directly or use available Codex UI question tools` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `ask the user directly or use available Codex UI question tools` confirms layout before Step 3
4. Step 3 — systems-designer spawned: specifies enemy compositions, loot tables, difficulty balance, area-specific mechanics, resource distribution; `ask the user directly or use available Codex UI question tools` confirms systems before Step 4
5. Step 4 — art-director and accessibility-specialist spawned in parallel; art-director: visual theme, color palette, lighting, asset list, VFX needs; accessibility-specialist: navigation clarity, colorblind safety, cognitive load check — each concern rated BLOCKING / RECOMMENDED / NICE TO HAVE; `ask the user directly or use available Codex UI question tools` presents both outputs before Step 5
...
9. Next steps listed: `$cgs-design-review design/levels/forest-dungeon.md`, `$cgs-dev-story`, `$cgs-qa-plan`
...
- [ ] `ask the user directly or use available Codex UI question tools` called at each step gate (minimum: after Step 1, Step 2, Step 3, Step 4)
...
- [ ] Next steps include `$cgs-design-review`, `$cgs-dev-story`, `$cgs-qa-plan`
...
4. `ask the user directly or use available Codex UI question tools` presented with options:
...
**Input:** `$cgs-team-level` (no argument)
...
3. Provides example invocations: `$cgs-team-level tutorial`, `$cgs-team-level forest dungeon`, `$cgs-team-level final boss arena`
...
4. Skill presents both Step 4 results via `ask the user directly or use available Codex UI question tools` — BLOCKING concern highlighted prominently
5. `ask the user directly or use available Codex UI question tools` offers:
...
- [ ] `ask the user directly or use available Codex UI question tools` presents the specific concern text (not just "accessibility issue found")
...
4. `ask the user directly or use available Codex UI question tools` presented with options:
   ...
   - (b) Pause and run `$cgs-team-level crystal caves` first to establish that area
...
- [ ] `ask the user directly or use available Codex UI question tools` offers a "design crystal caves first" option referencing `$cgs-team-level`
...
- [ ] `ask the user directly or use available Codex UI question tools` used at each step transition — user approves before pipeline advances
...
- [ ] Next steps present at end: `$cgs-design-review`, `$cgs-dev-story`, `$cgs-qa-plan`
