# Skill Test Spec: $cgs-team-polish
...
and issues READY FOR RELEASE or NEEDS MORE WORK). Uses `ask the user directly or use available Codex UI question tools` at each
...
- [ ] Has a next-step handoff at the end (references `$cgs-release-checklist`, `$cgs-sprint-plan update`, `$cgs-gate-check`)
...
2. `ask the user directly or use available Codex UI question tools` presents performance report; user approves before Phases 2, 3, and 4 begin
...
5. All three parallel phases complete; `ask the user directly or use available Codex UI question tools` presents results; user approves before Phase 5 begins
6. Phase 5: qa-tester runs edge case tests, soak tests, stress tests, and regression tests; all pass
7. `ask the user directly or use available Codex UI question tools` presents test results; user approves before Phase 6
...
- [ ] `ask the user directly or use available Codex UI question tools` appears after Phase 1 output and before Phases 2/3/4 launch
...
**Input:** `$cgs-team-polish particle-storm`
...
2. `ask the user directly or use available Codex UI question tools` presents the violation; user chooses to proceed with optimization attempt
...
9. Next Steps: schedule the remaining issue in `$cgs-sprint-plan update`; re-run `$cgs-team-polish` after fix
...
- [ ] Next Steps references `$cgs-sprint-plan update` for scheduling the remaining fix
...
**Input:** `$cgs-team-polish` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `$cgs-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`)"
...
**Input:** `$cgs-team-polish open-world`
...
3. `ask the user directly or use available Codex UI question tools` presents the performance report including the engine-level root cause; user approves before Phase 2
...
**Input:** `$cgs-team-polish inventory-ui` (Phase 5 scenario)
...
7. `ask the user directly or use available Codex UI question tools` presents the regression with options:
...
- [ ] `ask the user directly or use available Codex UI question tools` offers options including revert, fix in place, and schedule later
...
- [ ] Next Steps handoff references `$cgs-release-checklist` (on success) and `$cgs-sprint-plan update` + `$cgs-gate-check` (on failure)
