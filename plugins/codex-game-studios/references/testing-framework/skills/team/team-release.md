# Skill Test Spec: $cgs-team-release
...
- [ ] Has required frontmatter fields: `name`, `description`, `usage note`, `Codex-discoverable`, `tool expectations`
...
- [ ] Has a next-step handoff referencing post-release monitoring, `$cgs-retrospective`, and `production/stage.txt`
- [ ] Uses `ask the user directly or use available Codex UI question tools` at phase transitions requiring user approval before proceeding
...
**Input:** `$cgs-team-release v1.0.0`

**Expected behavior:**
1. Phase 1: Spawns `producer` via Task; confirms all milestone acceptance criteria met; identifies any deferred scope; produces release authorization; presents to user; ask the user directly or use available Codex UI question tools: user approves before Phase 2
2. Phase 2: Spawns `release-manager` via Task; cuts release branch from agreed commit; bumps version numbers; invokes `$cgs-release-checklist`; freezes branch; output: branch name and checklist; ask the user directly or use available Codex UI question tools: user approves before Phase 3
...
5. Phase 5: Spawns `producer` via Task; collects sign-offs from qa-lead, release-manager, devops-engineer; no open blocking issues; producer declares GO; ask the user directly or use available Codex UI question tools: user sees GO decision and confirms deployment
6. Phase 6: Spawns `release-manager` + `devops-engineer` (parallel); tags release in version control; invokes `$cgs-changelog`; deploys to staging; smoke test passes; deploys to production; simultaneously spawns `community-manager` to finalize patch notes via `$cgs-patch-notes v1.0.0` and prepare launch announcement
...
- [ ] `$cgs-changelog` is invoked by release-manager in Phase 6 (not written directly)
- [ ] `$cgs-patch-notes v1.0.0` is invoked by community-manager in Phase 6
...
4. Phase 4 proceeds cautiously or is paused (ask the user directly or use available Codex UI question tools: continue to Phase 4 or skip to Phase 5 for go/no-go?)
...
6. ask the user directly or use available Codex UI question tools: user is presented with the NO-GO decision and the S1 bug details; options: fix the bug and re-run, defer the release, or override (with documented rationale)
...
- [ ] ask the user directly or use available Codex UI question tools offers the user resolution options (fix and re-run / defer / override with rationale)
...
4. ask the user directly or use available Codex UI question tools: options presented — (a) Fix translations and re-run Phase 4, (b) Remove French locale from this release, (c) Ship as-is with a known issues note
...
- [ ] ask the user directly or use available Codex UI question tools is used to offer the user resolution choices — the skill does not auto-waive
...
**Input:** `$cgs-team-release` (no argument)
...
3. Confirms with ask the user directly or use available Codex UI question tools before beginning Phase 1 proper: "Releasing v1.1.0. Is this correct?"
4. Proceeds as if `$cgs-team-release v1.1.0` was the input

**Expected behavior (variant B):**
1. Phase 1: No argument provided; reads available state files — no version discoverable
2. Uses ask the user directly or use available Codex UI question tools: "What version number should be released? (e.g., v1.0.0)"
...
- [ ] Inferred version is confirmed with the user via ask the user directly or use available Codex UI question tools before proceeding (variant A)
- [ ] When no version is discoverable, ask the user directly or use available Codex UI question tools is used — skill does not guess (variant B)
...
- [ ] `ask the user directly or use available Codex UI question tools` used at each phase transition gate (post-Phase 1, post-Phase 2, post-Phase 3/4 if issues, post-Phase 5 go/no-go)
...
- [ ] Next steps always include 48-hour post-release monitoring, `$cgs-retrospective` recommendation, and `production/stage.txt` update to `Live`
...
- The "devops-engineer build fails" path is not separately tested — it would surface as a BLOCKED result in Phase 3 and follow the standard error recovery protocol (surface → assess → ask the user directly or use available Codex UI question tools options). This is validated structurally by the Static Assertions error recovery check.
...
- The "override NO-GO with documented rationale" path in Case 2 is referenced but not exhaustively tested — it is an escape hatch that the skill must support, and its existence is validated by the ask the user directly or use available Codex UI question tools options assertion in Case 2.
