...
- [ ] Uses `ask the user directly or use available Codex UI question tools` at phase transitions to capture user approval before proceeding
...
2. Phase 2: Spawns `qa-lead` via Task; produces strategy table classifying all 4 stories; no blockers flagged; presents to user; ask the user directly or use available Codex UI question tools: user selects "Looks good — proceed to test plan"
...
5. Phase 5: Spawns `qa-tester` via Task for each Visual/Feel and Integration story (2–3 stories); run in parallel; presents test cases grouped by story; ask the user directly or use available Codex UI question tools per group; user approves
...
- [ ] Next step: "Run `$cgs-gate-check` to validate advancement."
...
3. Skill reports: "Smoke check failed. QA cannot begin until these issues are resolved: [list of 2 failures]. Fix them and re-run `$cgs-smoke-check`, or re-run `$cgs-team-qa` once resolved."
...
- [ ] Skill recommends `$cgs-smoke-check` and `$cgs-team-qa` re-run as remediation steps
...
**Input:** `$cgs-team-qa sprint-05`

**Expected behavior:**
1. Phases 1–5 complete normally; test cases are written for the Visual/Feel story
2. Phase 6: User marks Visual/Feel story as FAIL; ask the user directly or use available Codex UI question tools collects failure description: "Animation plays at 2x speed — jitter visible on every loop"
...
7. Next step: "Resolve S1/S2 bugs and re-run `$cgs-team-qa` or targeted manual QA before advancing."

**Assertions:**
- [ ] FAIL result in Phase 6 triggers ask the user directly or use available Codex UI question tools to collect the failure description before the bug report is written
...
- [ ] Next step explicitly mentions re-running `$cgs-team-qa`
...
3. Proceeds as if `$cgs-team-qa sprint-06` was the input; reports "No sprint argument provided — inferred sprint-06 from session state. Found [N] stories."
...
2. Cannot infer sprint; uses ask the user directly or use available Codex UI question tools: "Which sprint or feature should QA cover?" with options to type a sprint identifier or cancel
...
- [ ] When both state files are absent, skill uses ask the user directly or use available Codex UI question tools rather than guessing (variant B)
...
- [ ] `ask the user directly or use available Codex UI question tools` used at Phase 2 (strategy review), Phase 5 (test case approval per group), and Phase 6 (per-story manual QA result)
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with ask the user directly or use available Codex UI question tools options
