# Skill Test Spec: $cgs-team-narrative
...
and world-builder in parallel). Uses `ask the user directly or use available Codex UI question tools` at each phase transition to
...
- [ ] Has a next-step handoff at the end (references `$cgs-design-review`, `$cgs-localize extract`, `$cgs-dev-story`)
- [ ] Error Recovery Protocol section is present
- [ ] `ask the user directly or use available Codex UI question tools` is used at phase transitions before proceeding
...
**Input:** `$cgs-team-narrative faction introduction cutscene for the Ironveil faction`
...
2. `ask the user directly or use available Codex UI question tools` presents the narrative brief; user approves before Phase 2 begins
...
4. `ask the user directly or use available Codex UI question tools` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `ask the user directly or use available Codex UI question tools` presents level narrative plan; user approves before Phase 4 begins
...
8. `ask the user directly or use available Codex UI question tools` presents review results; user approves before Phase 5 begins
...
- [ ] `ask the user directly or use available Codex UI question tools` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer Task calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `ask the user directly or use available Codex UI question tools` is approved
...
**Input:** `$cgs-team-narrative ironveil faction introduction cutscene`
...
6. `ask the user directly or use available Codex UI question tools` presents options:
...
- [ ] `ask the user directly or use available Codex UI question tools` presents at least 3 options including "stop and resolve first"
...
**Input:** `$cgs-team-narrative` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `$cgs-team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`)"
...
- [ ] No `ask the user directly or use available Codex UI question tools` is used — output is direct guidance
...
**Input:** `$cgs-team-narrative ironveil faction introduction cutscene` (Phase 5 scenario)
...
5. `ask the user directly or use available Codex UI question tools` presents options:
...
- [ ] `ask the user directly or use available Codex UI question tools` offers the option to fix now vs. flag and proceed
...
**Input:** `$cgs-team-narrative ironveil surrender negotiation scene`
...
- [ ] `ask the user directly or use available Codex UI question tools` offers at least one option to resolve the missing prerequisite
...
- [ ] `ask the user directly or use available Codex UI question tools` is used after every phase output before the next phase launches
...
- [ ] Next Steps handoff references `$cgs-design-review`, `$cgs-localize extract`, and `$cgs-dev-story`
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `ask the user directly or use available Codex UI question tools`
