# Skill Test Spec: $cgs-map-systems

## Skill Summary

`$cgs-map-systems` decomposes a game concept into a systems index. It reads the
...
- [ ] Has a next-step handoff at the end (`$cgs-design-system`)
...
2. File not found
3. Skill outputs: "No game concept found. Run `$cgs-brainstorm` to create one, then return to `$cgs-map-systems`."
4. Skill exits without creating systems-index.md

**Assertions:**
- [ ] Skill outputs a clear error naming the missing file path
- [ ] Skill recommends `$cgs-brainstorm` as the next action
...
- [ ] Ends with next-step handoff: `$cgs-design-system [next-system]`
...
- The `next` argument mode (handing off the highest-priority undesigned system to
  `$cgs-design-system`) is not tested here — it is a post-index-creation convenience.
