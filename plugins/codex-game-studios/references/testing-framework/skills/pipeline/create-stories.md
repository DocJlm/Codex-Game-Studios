# Skill Test Spec: $cgs-create-stories

## Skill Summary

`$cgs-create-stories` breaks a single epic into developer-ready story files. It reads
...
- [ ] Has a next-step handoff at the end (`$cgs-story-readiness`, `$cgs-dev-story`)
...
**Input:** `$cgs-create-stories [epic-name]`
...
- [ ] Blocking note names the specific ADR number and recommends `$cgs-architecture-decision`
...
**Input:** `$cgs-create-stories` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs a usage error: "No epic specified. Usage: $cgs-create-stories [epic-name]"
...
- [ ] Ends with next-step handoff: `$cgs-story-readiness` → `$cgs-dev-story`
...
- The story sizing rule (splitting large requirement groups) is not tested here
  — it is addressed in the `$cgs-create-stories` skill's internal logic.
