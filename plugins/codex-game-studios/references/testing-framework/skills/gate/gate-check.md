# Skill Test Spec: $cgs-gate-check

## Skill Summary

`$cgs-gate-check` validates whether the project is ready to advance to the next
...
**Input:** `$cgs-gate-check systems-design`
...
5. Skill suggests remediation: run `$cgs-brainstorm` to create one
...
- [ ] Output recommends `$cgs-brainstorm` as the remediation action
...
**Input:** `$cgs-gate-check` (no argument)
...
**Input:** `$cgs-gate-check systems-design` (with full mode active)
...
**Input:** `$cgs-gate-check systems-design` (with solo mode active)
...
ask the user directly or use available Codex UI question tools to confirm the auto-detected transition before running full checks —
