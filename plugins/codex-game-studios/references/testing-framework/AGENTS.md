...
| `quality-rubric.md` | Category-specific pass/fail metrics. Read the matching `###` section for the skill's category when running `$cgs-skill-test category`. |
...
| `results/` | Written by `$cgs-skill-test spec` when results are saved. Gitignored. |
...
Use `$cgs-skill-improve [name]`. It handles the full loop:
...
Nothing in `plugins/codex-game-studios/` imports from here. Deleting this folder has no effect on the
CCGS skills or agents themselves. `$cgs-skill-test` and `$cgs-skill-improve` will report that
